...
for (const contractStatus of ["ACTIVE", "TERMINATED"]) test(`new Billing accepts ${contractStatus} Customer and reads payment terms before writes`, async () => {
  ...
  assert.deepEqual(masterReads(state), [`${root}/Customers/customer`]); assert.equal(state.writes.length, 1); assert.equal(state.data.get(sourcePath).customerId, "customer");
});
for (const failure of ["missing", "other-tenant-only", "read-error"]) test(`new Billing Customers/customer ${failure} schedules no write`, async () => {
  const master = "Customers/customer";
...
test("new Billing does not read or require a live Site master", async () => {
  const state = runtime(); state.data.delete(`${root}/Sites/site`);
  await addOperationResultToBilling({ companyId: "company", doc: operation(), firestore: state.firestore });
  assert.equal(state.writes.length, 1); assert.equal(state.events.includes(`${root}/Sites/site`), false);
});
...
test("transaction retries repeat only Billing and payment-term Customer reads before create", async () => {
  ...
  assert.equal(state.events.filter((path) => path === sourcePath).length, 2); assert.equal(masterReads(state).length, 2); assert.equal(state.employeeReads().length, 0); assert.equal(state.writes.length, 1);
});
...
for (const contractStatus of ["ACTIVE", "TERMINATED"]) test(`move to absent Billing accepts ${contractStatus} Customer and reads payment terms before writes`, async () => {
  ...
  assert.deepEqual(state.events.slice(0, 2), [sourcePath, destinationPath]); assert.deepEqual(masterReads(state), [`${root}/Customers/customer`]); assert.equal(state.data.has(sourcePath), false); assert.equal(state.data.has(destinationPath), true);
});
test("absent-destination move retry rereads only Customer payment terms on every attempt", async () => {
  ...
  assert.equal(masterReads(state).length, 2); assert.equal(state.employeeReads().length, 0); assert.equal(state.writes.length, 2);
});
