...
async function client(options = {}) {
  ...
  const bindings = { ...Vue, ...contract, isBillingPaymentUxActorAllowed: contract.paymentActorAllowed, dateInput, useAuthStore: () => auth, useNuxtApp: () => ({ $firestore: {}, $functions: {} }), doc: (_, path) => ({ path }), getDocFromServer: async (ref) => { count++; if (options.read) return options.read(ref, count, raw); return { exists: () => true, data: () => raw }; }, httpsCallable: () => async (input) => { calls.push(input); return options.call ? options.call(input) : { data: { success: true } }; } };
