...
| Firestore data設計・Rules・Company CCB | 現行仕様の関連節、[ADR 0031](decisions/0031-proportional-data-boundary-and-change-safeguards.md)、[ADR 0066](decisions/0066-pre-production-document-level-last-write-wins.md)、[開発workflow](runbooks/development-workflow.md) | 全reader/writer、document serialization、通常更新・例外の競合制御、Rules、migration、rollback、対象環境・件数・停止条件 |
| page/component構成・useFetch・従属参照 | [Pageとcomponentの構成](specification.md#pageとcomponentの構成)、[表示dataと従属参照](specification.md#表示dataと従属参照)、[ADR 0067](decisions/0067-component-fetch-and-dependent-reference-boundary.md) | page/root/child、`useFetch` origin・inject、主対象listener、従属cache、missing表示、従属CRUと物理削除時検査 |
