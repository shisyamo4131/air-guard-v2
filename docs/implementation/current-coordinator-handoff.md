...
- CPU-05で静的caller 0の旧`CompanyManager`と`useSiteOrderManager`を削除し、Company rootのclient create/update/deleteを全面拒否した。同社の有効な本登録User readと4つの専用Callableは維持する。旧writer 0件のAST検査、全domain 726件、隔離Emulator 107件、一般review GO、security review 5/5を確認した。Codex UI smokeは起動templateのNuxt `ECONNRESET`で製品画面へ到達できず、主要画面再読込は未確認である。
...
- implementation and automated acceptance complete / UI reload pending checkpoint: `CCB-COMPANY-LEGACY-WRITER-REMOVAL-001`。旧Company全体writer 2件を削除し、Company rootのclient CUDを全面拒否した。4つの専用Callableと同社User readは維持する。全domain 726件、隔離Emulator 107件、一般review GO、security review 5/5は完了した。Codex UI smokeが起動templateのNuxt `ECONNRESET`で停止したため、CPU-05は主要画面再読込までIn progress・0点を維持する。
