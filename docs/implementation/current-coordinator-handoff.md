...
- specification: `0.8.4`
- unintegrated work: 0。STRIPE-02のapplication・Functions・Rules・package・test・主要文書はcommit `509fabbe77124b7bfe03b8b50c39ab9b6b488426`へ統合済みで、STRIPE-04の利用者用local migrationも完了した。Dev/Prod、remote data、deploy、外部Stripe、pushは未実施である。
...
- active product roadmap: [Company legacy Stripe情報削除](../roadmaps/company-stripe-removal.md) 70%。STRIPE-01の契約確定、STRIPE-02のlocal code・schema・test撤去、STRIPE-03の隔離Codex local migration準備、STRIPE-04の利用者用local migration・画面受入れを完了した。次は別承認が必要なSTRIPE-05のbounded Dev migration・反映である。[SuperUser兼会社管理者の表示順対応](../roadmaps/superuser-company-admin-display-order.md)は100%で完了済みである。
