...
- active product checkpoint: `CCB-COMPANY-PARTIAL-UPDATE-001`。まずCompany基本情報を専用editor・Callableへ移し、Class schema/operation validation、会社管理者境界、変更fieldだけの保存、server timestamp、編集中listener変更の通知、Rulesのclient直接変更拒否を固定する。
- next product checkpoint: 振込先、通常設定、取極め、表示順を同じoperation別構造へ順次移し、Company全体writer 0件を確認する。
