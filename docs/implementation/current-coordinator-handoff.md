...
- active product roadmap: [Company legacy Stripe情報削除](../roadmaps/company-stripe-removal.md) 50%。STRIPE-01の契約確定、STRIPE-02のlocal code・schema・test撤去、STRIPE-03の隔離Codex local migration準備を完了し、次は別承認が必要なSTRIPE-04のlocal migration・画面受入れである。[SuperUser兼会社管理者の表示順対応](../roadmaps/superuser-company-admin-display-order.md)は100%で完了済みである。
...
- active product checkpoint: `STRIPE-04`。利用者用Emulatorの対象はCompany 1件の`stripeCustomerId`・`subscription`削除だけで、`StripeData`は0件以外ならwrite 0で停止する。利用者は既存`./saved-data`を変更しないimport-only rehearsal、問題時のexportなし再開、合格後の同一baselineからのimport＋export-on-exit再実行・確認による確定を承認した。別backupと確定後のpre-migration data rollbackは要求しない。tooling・手順・独立review・最終検証を確定してから実dataへ進み、Dev・remote/data・外部Stripeは引き続き対象外とする。
