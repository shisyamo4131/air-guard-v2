...
- 配置管理の楽観的更新回帰は別branchで修正中である。clientの`functions/shared`直接参照を解消し、pending・queue・更新順保証・履歴を持たない単純なlocal即時反映を実装した。会社管理者のLocal UIで予定作成、作業員ドラッグ配置、listener反映後の表示維持を確認した。確認用予定とworkerはLocal Emulatorに残している。詳細は[FUT-0186](future-actions.md#fut-0186-配置管理の楽観的更新回帰を復旧する)を正とし、Functions、Rules、schema、保存形式は変更していない。
...
4マスターUIはDev非破壊smokeまで完了した。[FUT-0186](future-actions.md#fut-0186-配置管理の楽観的更新回帰を復旧する)は簡素化後のLocal予定作成・作業員ドラッグ配置を確認済みで、次は別承認によるDev反映・受入れへ進む。commit、main統合も別承認を待つ。完了後に[FUT-0185](future-actions.md#fut-0185-firestore-rulesの責務と式数を段階的に整理する)へ進む。入力component分離、tenant開放、Prodは別承認とする。
