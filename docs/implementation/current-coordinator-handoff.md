...
- next product checkpoint: `STRIPE-04`。承認済みlocal環境と対象dataを固定し、backup・digest・停止条件・rollbackを確認してlocal migrationと主要Company画面の読込・編集・再読込を受け入れる。自動開始せず、対象と外部作用について別の利用者承認を得る。Dev・remote/data・実環境反映は引き続き対象外である。
