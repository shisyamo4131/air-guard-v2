...
FGA-02は18点、phase全体は28%である。FGA-03の`FGA-03-SITE-NORMAL-AUTH-01`は通常作成・基本情報・Customer・Agreement・手動終了・再有効化のtenant-trust認可をcommit `ec46497adc70174db7b36a025fe7f477100f5568`へ固定した。[Local検証記録](../verification/fga-03-site-normal-auth-local.md)を正とする。Dev・Prodは未反映である。Site従属cacheのtenant分離は、通常sign-outではlayout再生成され、同一SPAの外部company claim変更は製品内未提供かつ実到達性未確認であるため、利用者判断によりFGA-03では実装しない。[FUT-0005](future-actions.md#fut-0005-サインアウト完了条件へmodel-cleanupを含める)のサインアウト／session切替共通cleanupへ統合した。次はSiteのManager／document LWW／Rules schema簡素化を分離し、最小checkpointを提示する。
