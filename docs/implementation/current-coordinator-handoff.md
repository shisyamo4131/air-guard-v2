...
1. Outsourcerは特定の協力会社masterであり、同じ外注先を配置へ複数回登録できる方式を維持する。旧試行の人数集約方式は採用しない。[Outsourcerロードマップ](../roadmaps/outsourcer.md)のOUT-01とOUT-02は、更新権限・破壊操作停止・保存data契約のlocal実装と検証を完了した。次はOUT-03の契約終了と候補の具体仕様を着手前に合意する。OUT-03以降、Dev/Prod、remote/dataは未承認である。
