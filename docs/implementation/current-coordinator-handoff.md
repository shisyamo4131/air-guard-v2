...
現在は[根本ガバナンス整合phase](../roadmaps/foundational-governance-alignment.md)の`FGA-06-RESULT-EDIT-NORMAL-AUTH-02`をLocal実装・自動検証・独立security reviewまで進め、phase全体は80%である。未確定かつlockされていない現場稼働実績の基本情報と従業員・外注先明細の編集だけをtenant共通のserver権限へ揃え、既存Callable、最新状態との照合、lock拒否、関連document更新を維持した。実績の作成・複製・削除、予定からの確定、配置通知、稼働外売上、請求操作、UI、Rules、schema、data migration、Dev・Prodは変更していない。次はLocal差分の利用者確認と固定commit化であり、Dev反映・受入れ、push、remote照合は別承認を必要とする。予定入力内の現場新規登録不具合は[FUT-0190](future-actions.md#fut-0190-現場稼働予定入力内の現場新規登録を復旧する)へ分離しており、未承認のまま修正しない。[実装記録](operation-result-edit-authorization.md)、[Local検証記録](../verification/fga-06-result-edit-normal-auth-local.md)、[FGA roadmap](../roadmaps/foundational-governance-alignment.md)を正とする。
