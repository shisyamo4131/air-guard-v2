...
現在は[根本ガバナンス整合phase](../roadmaps/foundational-governance-alignment.md)の`FGA-06-RESULT-EDIT-NORMAL-AUTH-02`をDev受入れまで完了し、phase全体は80%である。lockされていない実績の基本情報と従業員・外注先明細の編集をtenant共通のserver権限へ揃え、既存Callable、最新状態照合、lock拒否、関連更新を維持した。release `4bd80e3f`のActions #30でFunctionsだけを反映し、合成tenantの通常UIで基本情報変更と再表示を確認した。実績は削除、合成Siteはアーカイブし、両一覧を0件へ戻した。別actor、worker編集、拒否経路はLocal自動検証を正とする。作成・削除等の対象外契約、UI、Rules、schema、migration、Prodは変更していない。次はFGA-06の残るtransaction系機能から小checkpointを提案する。[実装記録](operation-result-edit-authorization.md)、[Local検証](../verification/fga-06-result-edit-normal-auth-local.md)、[Dev受入れ](../verification/fga-06-result-edit-normal-auth-dev.md)を正とする。予定入力内の現場新規登録不具合は[FUT-0190](future-actions.md#fut-0190-現場稼働予定入力内の現場新規登録を復旧する)へ分離済みである。
