...
FGA-02は18点、phase全体は28%である。FGA-03の4commitを`7a13d151f3ad6bd2e68d021cd0f7ecd6e3a630e6`へ固定し、GitHub Actions #13でFirestore・Functions・HostingへDev反映した。Codex専用合成tenantで、3ステップからCustomer未登録Siteの作成、通常更新、終了、再有効化、archive、Agreement追加・再読込後の再表示、Agreement保持Siteのarchiveと通常一覧0件への復帰を確認した。[Local検証記録](../verification/fga-03-site-manager-lww-local.md)と[Dev受入れ記録](../verification/fga-03-site-manager-lww-dev.md)を参照する。安全に利用できる別権限accountの既存sessionがなく、別actorのDev実操作は未確認である。利用者の主観受入れも未確認であり、FGA-03全体と進捗28%は未完了のままとする。tenant拒否とapplicationを介さない直接requestは今回想定しない範囲であり、直接requestへの追加対策とarchive形式変更は本phaseへ含めない。Site従属cacheはFGA-03で実装せず、[FUT-0005](future-actions.md#fut-0005-サインアウト完了条件へmodel-cleanupを含める)のサインアウト／session切替cleanupへ統合した。
