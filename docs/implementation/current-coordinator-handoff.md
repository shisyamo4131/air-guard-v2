...
4. 原因は、予定Managerで画面本体用の`table`表示口と汎用転送の同名表示口が重複し、空の方が本来の左一覧・右詳細・日報写真を上書きしていたことです。汎用転送から`table`を除外するLocal補正と回帰test、全domain、buildは完了しています。次は補正版をcommitしてGitHub ActionsからDevへ再反映し、表示を確認した後、配置管理と上下番確定処理そのもののエラーを別々に再現します。
