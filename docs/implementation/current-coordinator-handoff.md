...
FGA-02は18点、phase全体は28%である。FGA-03通常認可はcommit `ec46497adc70174db7b36a025fe7f477100f5568`へ固定した。[Local検証記録](../verification/fga-03-site-normal-auth-local.md)を参照する。通常Siteのschema・業務検査を正規application writerへ集約する`FGA-03-SITE-RULES-SIMPLIFY-04`はLocal検証済みで、tenant・identity・lifecycle・Customer・delete・archive保護を維持する。[Rules簡素化Local検証記録](../verification/fga-03-site-rules-simplification-local.md)を正とする。applicationを介さない直接requestへの追加対策とarchive形式変更は本phaseへ含めない。Dev・Prodは未反映で、次は候補のcommitとDev受入れ、またはSite Manager／document LWWである。Site従属cacheはFGA-03で実装せず、[FUT-0005](future-actions.md#fut-0005-サインアウト完了条件へmodel-cleanupを含める)のサインアウト／session切替cleanupへ統合した。
