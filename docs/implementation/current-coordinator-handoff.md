...
現在は[根本ガバナンス整合phase](../roadmaps/foundational-governance-alignment.md)のFGA-02を完了し、FGA-03 Site管理を開始した。現在地と次操作は23行目を正とする。CompanyとUserは現行の厳密な実装を維持する。Customer Manager最終形はmerge commit `7d82996652d2d65448cf7eff9cd7e1ecc5457ae2`をDevへ反映・受入れ済みである。[最終Dev証拠](../verification/fga-02-customer-manager-simplification-dev.md)を参照する。合成Customer `FGA02001`と`FGA02MGR02`は保持し、`FGA02REL11`はarchive済みである。
...
FGA-02は18点、phase全体は28%である。FGA-03通常認可はcommit `ec46497adc70174db7b36a025fe7f477100f5568`へ固定した。[Local検証記録](../verification/fga-03-site-normal-auth-local.md)を参照する。`FGA-03-SITE-RULES-SIMPLIFY-04`はLocal検証済みで、`FGA-03-SITE-MANAGER-LWW-05`も利用者訂正を反映してLocal検証済みである。通常CREATE・UPDATEは共通ManagerからSite modelの標準保存へ直接接続し、通常保存用`useSiteActions`と専用writerを撤去した。作成は3ステップ入力とCustomer未登録時の仮登録を維持する。取極め・終了・再有効化・archiveの専用操作と、tenant・identity・Customer・delete・archive保護を維持する。applicationを介さない直接requestへの追加対策とarchive形式変更は本phaseへ含めない。Dev・Prodは未反映である。Site従属cacheはFGA-03で実装せず、[FUT-0005](future-actions.md#fut-0005-サインアウト完了条件へmodel-cleanupを含める)のサインアウト／session切替cleanupへ統合した。
