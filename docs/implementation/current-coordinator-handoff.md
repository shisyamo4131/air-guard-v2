...
Manager訂正と一般化したgovernanceはmerge commit `2eeb502bf163a5952f24a02a2e2f5da58ac26df6`として`main`へ統合し、GitHub Actions run #10でHostingへDev反映した。会社管理者の外部Chromeでは、最初に再読込した後、一覧CREATE、listener反映、`CustomersManager`／AirArrayManagerの`beforeEdit`による詳細navigation、単数`CustomerManager`の詳細UPDATE、両dialogの実測480px、更新後と再読込後の表示を確認した。[Dev受入れ記録](../verification/fga-02-customer-manager-dev.md)を参照する。現行routeにはAutocompleteのcreatable callerがないため、そのruntimeは未確認である。次はAutocomplete runtimeを現行checkpointの必須条件として残すか、到達可能なcaller追加を別checkpointにするかを利用者と確定し、Customer archive・失敗経路の残件を閉じる。FGA-02得点は0のままとする。
