...
- 製品は試験運用中。Customer状態改修のCS-03までを完了し、CS-04のDev反映・受入れをマスタデータ管理改修後まで延期しています。Customer archive safetyはCAS-03までlocal実装・検証・Git統合を完了し、CAS-04のclient/UIが次のcheckpointです。
...
- 現在の製品checkpointはCustomer archive safetyのCAS-04。[確認済み仕様](../specification.md)、[ADR 0046](../decisions/0046-customer-archive-reference-barrier.md)、[工程・進捗](../roadmaps/customer-archive-safety.md)、[実装設計](customer-archive-safety.md)から再開する。
- Spark用standalone Developer taskは実装前に中止・削除済みで、再利用しない。CAS-02/03はprimary coordinator配下の通常サブエージェント運用で完了した。CAS-04のlocal client/UI・関連test・Codex専用build/UI・独立review・local Git統合は承認済み。CAS-05、Dev/Prod、remote data/migration、package、restore、retention/purge、code一意性/検索拡張、他マスタは対象外。
...
- CAS-04では、client/UI実装、domain/source-contract test、Codex専用local UI受入れ、独立reviewを非重複ownershipで通常サブエージェントへ分割し、coordinatorが外部操作・差分受入れ・Git統合を担う。
...
1. CAS-03の安全境界を前提に、CAS-04のCustomer詳細archive action、確認dialog、二重送信防止、安全なerror表示、成功後一覧遷移を実装し、source-contract/domain/専用build/専用local UI/独立reviewで確認する。
