...
>
> 2026-09-01 STRIPE-02 current note: AirGuardV2 root/FunctionsはSchemas exact `3.0.0-dev.1`へ移行し、checkout、legacy subscription reader、未公開Stripe Functions、Stripe依存をlocal codeから削除した。`StripeData`は全actor・全階層で拒否する。以下にある`.166/.167`、Stripe field、checkout、旧Functionsの記述は当時のrollback inventoryであり、現在のruntime事実ではない。Admin SDKは引き続き`.167`で、既存data migration・Dev反映は未実施である。
>
...
| gcloud trust/token refresh | [Windows trust手順](../runbooks/dev-deployment/authentication-and-windows.md#gcloudを使う場合のwindows-trust)のPython `truststore.inject_into_ssl()`付き`gcloud firestore databases describe` exact process | Dev `(default)` metadata取得成功 | 0 |
