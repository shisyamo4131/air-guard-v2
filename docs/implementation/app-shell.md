...
- 同ミドルウェアのコメントには「未認証の場合は `public: true` のページのみアクセス可能」とある一方、実装はページ設定が存在しない場合を許可する分岐を持つ。さらに `getPageConfig` の親探索は未登録パスから登録済み `/` まで遡り、ホームの `public: true` 設定を返し得るため、設定なし分岐へ到達する前に公開ページとして扱われる場合がある。コメントと実装の記述は一致していない。
