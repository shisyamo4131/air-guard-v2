...
`.github/workflows/dev-deploy.yml`が`main`へのpushを受け、`scripts/select-dev-deploy-targets.mjs`で変更fileをFirebase serviceへ対応付ける。GitHub `dev` Environmentは`main`だけを許可する。Google Cloudの専用service accountはGitHub OIDCとWorkload Identity連携で一時的に借用し、JSON秘密鍵をGitHubへ保存しない。
...
- Secrets: `NUXT_PUBLIC_FIREBASE_API_KEY`、`NUXT_PUBLIC_FIREBASE_AUTH_DOMAIN`、`NUXT_PUBLIC_FIREBASE_DATABASE_URL`、`NUXT_PUBLIC_FIREBASE_PROJECT_ID`、`NUXT_PUBLIC_FIREBASE_STORAGE_BUCKET`、`NUXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID`、`NUXT_PUBLIC_FIREBASE_APP_ID`、`NUXT_PUBLIC_FIREBASE_REGION`、`NUXT_PUBLIC_FIREBASE_VAPID_KEY`
...
6. deploy jobではDev識別子検査、必要な依存install、Hosting生成、鍵なし認証、Firebase CLI version、dry-run、実deployの順に各stepを確認する。
