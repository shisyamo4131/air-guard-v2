...
3. 利用者へ対象commitと予想serviceを示し、`main` pushの承認を得る。この承認は自動選択されたDev deployまでを含む。
   - Functionsのexportを廃止した場合の単発削除は、[共通runbookの直接削除手順](../dev-deployment.md#廃止済みcloud-functionの単発削除)で対象を示して個別承認を得る。削除だけのために通常workflowを編集・復元せず、deploy全体への`--force`で代用しない。
...
- Functions削除の承認がない、または削除対象が事前確認と異なる: その削除を伴う実deployを開始せず、[直接削除手順](../dev-deployment.md#廃止済みcloud-functionの単発削除)へ戻る。workflowへの一時step追加や包括的な`--force`で回避しない。
