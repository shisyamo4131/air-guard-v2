...
4. [フェーズごとのテスト範囲の合意](../project-rules/development-and-data.md#フェーズごとのテスト範囲の合意)と[3環境の選択基準](../project-rules/environment-and-approval.md#local-emulatorとlocal-ui)に従い、自動検証で残る不確実性を直接減らす場合だけLocal UI・利用者環境検証を選ぶ。自動検証で十分な変更はこの任意確認を省略できる。
5. 小工程の最終状態では影響範囲の回帰と、その段階のcompletion gateを満たす。Local Emulator全件testは各小工程で重ねず、複数工程を含む最終Dev候補差分が固まった時点へ集約する。[全件確認と対象限定確認](local-emulator-testing.md#全件確認と対象限定確認)に従い、初回全件成功後は非失効結果を再利用し、失効・不足範囲だけを再実行する。影響不明・基盤全体への波及・明示例外は小工程でも全件確認する。上位gateに含まれる下位gateも重ねない。
