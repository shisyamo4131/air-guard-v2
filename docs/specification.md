...
- Firestoreのclient書込み境界はcollection名だけで一律に決めず、field ownership、整合性、監査、同時実行、offline、複数document、server-only値、外部作用を確認して機能単位で見直す。通常の可逆操作は、上記のactor・tenant境界と最低限の破壊防止条件をRulesで強制できる場合にDomain ManagerからFireModel／ClientAdapterの標準client保存へ接続することを既定候補とする。既存Callable、transaction data、server実装の存在だけを維持理由にしない。通常業務でも複数documentのatomicity、server-only値、外部作用、冪等性、再開・reconcile等の技術要件によりCallableを使用できるが、operationごとにclient transaction・batch・triggerでは満たせない理由と最小server所有範囲を示し、その技術要件だけからrole制限やschema検査を戻さない。判断理由と既存仕様の置換範囲は[ADR 0065](decisions/0065-tenant-trust-normal-business-authorization.md)と[ADR 0071](decisions/0071-normal-business-manager-and-callable-boundary.md)を正とする。
...
- 現場稼働予定の作成・複製・基本情報変更・配置作業員変更・表示順変更・削除は通常業務とし、同じtenantの有効な認証済み本登録Userに同じserver権限で許可する。role、permission、会社管理者、super-user区分を認可根拠にせず、仮登録、無効User、User不在、claim不正、他tenantを拒否する。複数documentの整合性、Siteの予定revision、配置通知の取消し、実績化済み予定の変更・削除拒否を維持するため、正式保存境界は専用`saveOperation` Callableのままとする。配置通知の作成と現場稼働予定から稼働実績への確定はこの通常C/U/D権限へ含めず、現行のserver側actor条件を維持する。
...
- 同じtenantの有効な本登録Userは、ロックされていない稼働実績の基本情報と従業員・外注先明細をroleに依存せず編集できる。稼働実績の作成・複製・削除はこの共通権限へ含めず、既存の`operation-results:write`等によるserver認可を維持する。`operation-billings:write`を持つ利用者は請求項目を編集し、`isLocked`を設定・解除できる。
