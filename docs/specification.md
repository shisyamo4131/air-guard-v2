...
- 取引先のclient直接deleteと`Customers_archive`のclient read・作成・変更・削除は許可しない。取引状態の編集と、参照確認・監査を伴うarchive・緊急restoreは別の操作とする。専用archive Callableとarchive非公開・参照barrierはlocal実装済み・未deployで、archive画面入口と緊急restoreは未実装である。対応する画面入口はCAS-04の確認操作を実装するまで提供しない。単なる状態フラグという扱いを削除・復元の安全条件へ拡張しない。
