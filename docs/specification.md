...
- 取引先のclient直接deleteと`Customers_archive`のclient作成・変更・削除は許可しない。取引状態の編集と、参照確認・監査を伴うarchive・緊急restoreは別の操作とする。archive・緊急restoreは未実装であり、対応する専用操作を実装するまで画面入口を提供しない。単なる状態フラグという扱いを削除・復元の安全条件へ拡張しない。
