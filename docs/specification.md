...
- role presetの識別子、表示metadata、permission配列は`@shisyamo4131/air-guard-v2-schemas/constants`を環境非依存の単一正本とし、ルートアプリとCloud Functionsは同じ公開version・tarball・integrityを使用する。現在のconsumer versionはexact `3.0.0-dev.3`である。このpackage catalogはactor・tenant・target・request contextからallow/denyを決定せず、clientとFunctionsがそれぞれ認可policyを所有する。
- Schemas packageを更新する前後は、当該turnでsource tag manifest、repository release evidence、AirGuardV2 root/Functionsのmanifest・lockにあるname、version、resolved、integrityを機械照合する。prompt、chat、要約、agent reportだけでpackage identityを確定せず、矛盾時はconsumer file変更・install・testを開始しない。`3.0.0-dev.3`はroot/Functionsへ同時導入し、`PostAdoption`で同一version・tarball・integrityを確認した。saveOperationのnotify生成分岐もSchemasの`actualIsStartNextDay` parityへ補正し、直接対象test、domain-full 1433/1433、Local Emulator 180/180、最終Local T21、TESTERの会社管理者Local UI、ユーザー本人の一方向遷移・LEAVED終端表示を確認した。DEV read-only確認では配置通知2579件と関連予定・勤務実績の整合を確認し、migration/repair不要と判断した。Dev/Prod反映、FCM実配信、backend日付算術、dashboard本人表示、Dev受入れは未検証で、SCR-02はDev受入れ前の未完である。詳細は[SCR-02 Local検証記録](verification/scr-02-arrangement-notification-local.md)を参照する。旧`3.0.0-dev.2`、`3.0.0-dev.1`、`2.4.2-dev.167`は履歴保持する。
...
- Firestore上の通常業務dataについて製品が提供するdocument create・update・deleteは、master dataかtransaction dataかを問わず、編集可能stateの所有単位に対応するdomain Manager componentを画面上の共通入口とする。外部から渡された既存の単一instance、またはその場で生成した新規instanceを編集する文脈では単数形domain Managerが`AirItemManager`をラップし、collection・list自身が配列と行選択のdispatchを所有する文脈では複数形domain Managerが`AirArrayManager`をラップする。Customerの`CustomerManager`／`CustomersManager`、Siteの`SiteManager`／`SitesManager`、Employeeの`EmployeeManager`／`EmployeesManager`、Outsourcerの`OutsourcerManager`／`OutsourcersManager`を最初の適用例とし、予定・実績・請求等はFGA-06で提供操作と例外を確認して段階移行する。画面が選択UIであること、またはdataがtransactionであることだけを複数形Managerの根拠やManager不使用の理由にしない。各Managerはそのstate所有文脈で製品が提供するoperationだけを扱い、単数・複数形の双方に全C/U/Dを要求しない。
...
- 通常業務dataのCRUDはmaster・transactionを問わずdocument全体を編集・検証・保存する。[Pageとcomponentの構成](#pageとcomponentの構成)に定めるdomain Managerの入口・保存経路・例外を適用する。
...
- マスタdata等の参照元documentを物理削除する場合は、操作時点で既知の従属documentを確認し、一件でも存在する、検査に失敗する、または対象が不整合なら中断する。従属側CRUとのlock、tombstone、transactional barrierは既定にせず、検査の直前または同時に従属documentが生じるraceまで保証しない。transaction dataの物理削除にはこの削除前検査を一律適用せず、関連dataの削除・更新・再集計をTriggerへ委譲する。残存riskを強い整合性を保証したとの表示・監査記録へ読み替えない。このtrade-off、現行実装差、例外境界は[ADR 0067](decisions/0067-component-fetch-and-dependent-reference-boundary.md)と[ADR 0072](decisions/0072-transaction-delete-client-trigger-boundary.md)を正とする。
...
- 新規作成・座標取得対象住所の変更で座標取得だけが失敗しても、入力・認可・operation固有validationを満たした住所は保存する。古い住所の座標を残さず未取得とし、保存成功後に「住所は保存済み、座標は未取得」と通知する。Firestore保存自体の失敗や結果不明を住所保存成功と表示しない。
...
- Employeeの退職・誤退職訂正は標準CRUD化の例外とし、既存Callableの単一経路を維持する。退職はUser連携の有無をclientで分岐せず、Callable内で予約と実際のUserを照合し、保存時にも再確認する。Userなしの場合も同じCallableでEmployeeの退職状態と操作履歴を保存する。本登録User連携時はUser/Auth削除・予約解放・後処理と途中失敗からの再開を管理する。同社の有効な本登録会社管理者・統括・人事という既存の退職actor条件、自己操作・管理者等の対象guard、仮登録User連携時の拒否、認証・tenant検証は維持する。適用確認は[SCR-09](roadmaps/standard-crud-alignment.md#scr-09-employee退職誤退職訂正の目的と完了条件)で管理する。
...
- User連携の有無によらないEmployee退職・誤退職訂正と、単独本登録User削除の実行状態と監査は、server-onlyの`LifecycleOperations`を操作単位の唯一の正本とする。operation ID、対象別lock、Auth削除前の永続的intent、phase、再試行、部分失敗reconcile、完了結果を保持し、clientからの直接read/writeを許可しない。email、email hash、role、通知設定、User/Auth全文、FCM tokenは保存しない。actor UIDと最大6文字の表示名、target UIDと最大6文字の表示名、Employee ID、退職日・現在上限20文字の退職理由、単独User削除理由を操作種別に必要な最小snapshotとしてserver-onlyで保持する。 この操作履歴の要件をEmployeeの通常情報編集へ拡張しない。
...
- 誤退職訂正は既存Callableで行い、実行者を会社管理者に限定する。対象EmployeeがRESIGNEDでUser連携がなく、訂正元が最新かつ後処理まで完了した退職履歴であり、別のlifecycle処理中でないことを保存transaction内で検証する。同じEmployeeをACTIVEへ戻して退職日・理由を削除し、訂正履歴と最新操作の参照を同じtransactionで保存する。同一操作の再実行による重複を防止し、旧User/Authは自動復元しない。既存operation・監査記録を保持し、実際の退職期間を伴う再雇用の業務条件は別事項のままとする。
...
- CCBの主目的は、認証・tenant管理の基点であるCompany documentを現在の厳密なactor・field・validation・競合制御で維持しつつ、機微・機密情報とserver-only情報を通常情報から分離することである。Companyをtenant共通権限またはdocument単位last-write-winsへ移行しない。
- Companyの基本情報、通常設定、表示順、振込先等は、現在の専用editor、field別writer、Callable、actor条件、validation、listener同期、競合時の停止・再読込契約を維持する。`AirItemManager`または`AirArrayManager`の共通部品を利用する場合も、Company document全体のwriteへ広げない。
- document全体の必須・型・長さ・相関はFireModel/Class schema、operation固有条件は既存の共有operation contractを正本とする。保存前は最新Companyへ変更fieldを重ねたcandidateを検証し、operation所有fieldと必要なserver管理・派生fieldだけを保存する。
- editorはreal-time listenerのCompany本体と独立したdraftを使い、同じoperation所有fieldの外部更新を検出した場合は現在の競合通知、保存停止、再読込または明示再確認契約を維持する。自分の保存結果を外部競合として表示しない。
...
- 振込先更新は専用`updateCompanyBilling` Callableを使い、requestをnon-emptyな`changes`と5 fieldのsubsetへ限定する現在のvalidation・identity・actor・client直接write拒否を維持する。分割実装ではtenant pathを検証済みidentityから導出し、別documentの最新値を基準に相関を検証して、実際に変わったfieldとserver管理metadataだけを保存する。Company rootの`invoiceNumber`は振込先requestで受信・更新しない。現行CallableがCompany rootを更新する実装は移行checkpointで置換する。
- 完全な振込先だけを口座名義を含めて帳票へ印字し、不完全または不正な振込先は印字しない。会社情報の表示・帳票layoutは長い値を折返し、縮小または表示上の省略で扱い、保存値またはsnapshot値を切り捨てない。100文字の会社名、長い住所・建物・口座名義をrender test対象とする。field validationと更新境界は[ADR 0033](decisions/0033-company-bank-transfer-update-boundary.md)、document分割は[ADR 0064](decisions/0064-sensitive-firestore-document-boundaries.md)を正とする。
...
- `arrangement`の`siteOrder`と`scheduleOrder`は各最大2000件とし、各itemはexact `{siteId, shiftType}`だけを持つ。`siteId`は1〜128文字で`/`とcontrol characterを拒否し、`shiftType`は`DAY/NIGHT`、同一配列内の`siteId + shiftType`は一意とする。computed `key`は保存しない。actorは同じtenantの有効な本登録Userとし、会社管理者、またはnon-super-userかつ既知role preset由来で`siteOrder`には`sites:write`、`scheduleOrder`には`site-operation-schedules:write`を持つUserだけを許可する。会社管理者は`isSuperUser === true`でも自社の両表示順を更新できる。会社管理者でないsuper-user、直接permission、未知role、temporary、disabled、他tenantは拒否する。専用Callable、field限定保存、競合時の停止・再読込を含む現行契約を維持する。全Siteの存在、型・上限、表示、削除済み参照、保存中のUX条件も維持する。actor境界は[ADR 0037](decisions/0037-superuser-company-admin-display-order.md)、Site表示判断は[ADR 0036](decisions/0036-terminated-site-display-order-visibility.md)を正とする。
...
2026-09-15の質疑応答で確定し、2026-09-16にEmployee退職・誤退職訂正の例外を改訂した。以下は承認済みの保存方式であり、実装の適用状態は[CRUD棚卸し](implementation/operation-crud-simplification-inventory.md)を参照する。旧来の「状態変更・請求確定・実績化だから専用Callable」という規定を置換する。

- 取引終了・再開など通常のマスター業務状態変更は、ManagerとSchemasクラスの標準更新を使う。Employeeの退職・誤退職訂正は例外として既存Callableの単一経路を維持し、User連携の判定、退職・訂正履歴、認証account操作との整合性をserverで検証する。その他の認証account、User、tenant管理の変更も厳密な専用処理を維持する。業務上の状態・入力条件と保存経路は区別し、終了・退職後の通常情報編集など未変更の提供条件を一律撤去しない。
...
- Outsourcerの閲覧、作成、基本情報変更、`contractStatus`変更は通常業務とし、認証UIDとUser document IDが一致する同じtenantの有効な認証済み本登録Userに同じserver権限で許可する。role、permission、会社管理者、super-user区分を認可根拠にせず、仮登録、無効User、User不在、claim不正、他tenantは拒否する。
...
- 作成時の`docId`はpathと一致させ、`uid`は実行者、`createdAt/updatedAt`はrequest時刻とする。更新時は`docId/createdAt`を変更せず、整合したdocument全体と正規の`uid/updatedAt`を保存する。名称系`name/nameKana/displayName`を変更した場合は検索用`tokenMap`を同じdocumentへ再生成する。RulesはtokenMapを最大512件・値trueだけに制限するが、名称との完全な意味的一致までは再計算できないため、正規画面の保存経路を維持する。
- Outsourcerの通常更新はdocument単位のlast-write-winsとし、同時更新を理由に保存拒否、競合通知、再読込要求を行わない。更新後はlistenerの最新documentを正本とする。単数形Managerは一つの既存instanceまたはその場の新規instance、複数形Managerは一覧配列と行選択を所有し、modelの標準`create()`／`update()`で保存する。
