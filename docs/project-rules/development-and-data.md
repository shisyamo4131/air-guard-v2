...
- 単数domain Managerのactivatorはbase Managerの`toCreate`、`toUpdate`、製品が提供する場合だけ`toDelete`をoperation別に公開し、callerが目的に対応するmethodを呼ぶ。domain wrapper独自の`operation` propまたは単一`open`関数でedit modeを再管理しない。wrapperはfreshなcreate instanceの生成、listener由来update instanceの接続、許可operation、domain handler・error表示等の薄い境界を持てるが、base Managerのmode、validation、submitを複製しない。複数形Managerはheader・tableの標準dispatchと`beforeEdit`を使い、`handleClickCreate`・`handleClickUpdate`・`handleClickDelete`でbase dispatchを迂回する場合は確認済み理由を示す。
