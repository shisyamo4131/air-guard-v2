...
| 認証・認可・テナント分離 | [ADR 0002](../decisions/0002-multitenant-firebase-architecture.md)、[ADR 0014](../decisions/0014-codex-dedicated-local-test-data.md)、[ADR 0016](../decisions/0016-firemodel-crud-boundary.md) | Rules、認証・管理者処理、Codex専用local基盤 | セキュリティレビュー、Auth・Firestore・Storage・Callable 55件、User更新Auth同期、有効化・無効化Callable、会社管理者移譲、本登録基盤の単体test。一般User本登録はlocal Emulatorの合成Userで仮登録照合、Auth作成、メール確認、本登録Callable、claim反映、dashboard到達を確認済み。Firestore・Storage tenant identity gate、再構築Callable gate、全会社メール重複確認の会社管理者gate、signup用メール利用可否handlerの既存分岐は確認済み。残るCallableとDev・remote受入れは未完了 |
