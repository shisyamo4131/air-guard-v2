...
3. Outsourcerは[専用ロードマップ](outsourcer.md)で管理する。利用者は、Outsourcerを特定の協力会社masterとし、同じ外注先を配置へ複数回登録できる現行方式を維持して、旧試行の人数集約方式を採用しないと確定した。OUT-01からOUT-05の更新権限・保存data・status・非archive・検索表示に加え、OUT-06で重複配置を安定した`workerId`の別明細として扱うlocal実装・契約検証を完了し、専用ロードマップで80%を加点した。OUT-07 local統合確認は利用者承認済みで進行中であり、完了条件を満たすまで追加加点しない。OUT-08とDev反映は別承認である。
4. マスタ管理機能の改修中は対象masterのCRUDを主対象とする。配置・通知・稼働実績・請求・帳票などtransaction系機能への波及変更は、Firestore更新に関係しない表示・読取り・描画identityの互換修正に限定する。transaction系の要改修箇所を検出しても実装せず、既知課題として記録する。マスタ管理の一連の改修後に、OperationResultの管制側編集lockと権限境界、Billing/勤怠/履歴同期、rounding、notification、配置更新失敗時のrollback/refetchを含む改修範囲を別checkpointで合意する。
