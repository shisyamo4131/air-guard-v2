...
3. Outsourcerは[専用ロードマップ](outsourcer.md)で管理する。利用者は、Outsourcerを特定の協力会社masterとし、同じ外注先を配置へ複数回登録できる現行方式を維持して、旧試行の人数集約方式を採用しないと確定した。OUT-01の更新権限・破壊操作停止、OUT-02の保存data契約、OUT-03のstatus非制限はlocal実装・検証を完了し、専用ロードマップで40%を加点した。OUT-04以降とDev反映は未承認であり、本ロードマップ本体の進捗には加点しない。
