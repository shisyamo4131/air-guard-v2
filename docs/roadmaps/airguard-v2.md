...
6. 4マスターUI branchのDev受入れとmain統合後、別branchで配置管理の楽観的更新回帰を最優先でreview・修正する。予定自体、配置作業員、通知の作成・更新・削除が操作直後に画面へ反映され、server拒否・timeout時にrollback、正本再取得、error表示、明示再試行へ収束することを必須条件とする。専用Callable化が原因かは未確認であり、[FUT-0186](../implementation/future-actions.md#fut-0186-配置管理の楽観的更新回帰を復旧する)で実行経路を確認してから修正する。
