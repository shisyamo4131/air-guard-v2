...
4. Siteは[専用ロードマップ](site.md)で管理する。SITE-08で先行のSite lifecycle・archive・actor境界に加え、利用者許可のLocal Chromeで背景trigger同期、基本・Customer・取極め編集、既存snapshot不変、予定更新・実績化を確認した。旧Site更新と予定プリセットの回帰を修正済みで、最終build・cleanup・統合までは90%を維持する。SITE-09のDev反映・remote/data確認は別承認である。
