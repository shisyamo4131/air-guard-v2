...
| 認証・認可・テナント分離 | 20 | 0 | Verification required（要検証） | UWB-05で本人プロフィール、通知、roleのfield別Callableを実装し、domain単体test 490件と専用Emulator 79件が成功した。利用者によるUWB-05 application file・local UI受入れ、UWB-06以降、Users blanket Rulesを含むUWB-08、App Check、Dev・remote受入れは未完了でdeploy不可。 |
...
1. Auth claim・tenant path・User状態の認可整合性を最優先で継続する。UWB-05のfield別Callable実装と自動検証は完了した。次は利用者によるUWB-05 application file・local UI受入れを完了し、UWB-06、UWB-07の具体例壁打ち、UWB-08のUsers Rules閉鎖へ進む。App Check・rate limit、Dev受入れも未完了であり、このgate全体の完了までdeployしない。
