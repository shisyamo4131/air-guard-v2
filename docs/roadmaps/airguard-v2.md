...
| 認証・認可・テナント分離 | 20 | 0 | Verification required（要検証） | UWB-04の予約lifecycleとUI受入れ後、仮登録操作を`users:provision`、role等の管理を`users:write`へ分離した。domain単体test 468件、専用Emulator 74件、human-resource正規UIでrole control不存在・roleなし作成・削除を再検証した。UWB-05以降、Users blanket Rulesを含むUWB-08、App Check、Dev・remote受入れは未完了でdeploy不可。 |
...
1. Auth claim・tenant path・User状態の認可整合性を最優先で継続する。UWB-02R〜04Rの自動検証とhuman-resource正規UI再受入れは完了した。次はUWB-05〜06、UWB-07の具体例壁打ち、UWB-08のUsers Rules閉鎖へ進む。App Check・rate limit、Dev受入れも未完了であり、このgate全体の完了までdeployしない。
