...
6. 配置管理の楽観的更新回帰は別branchで実装し、Local UI確認前にclientから`functions/shared`への直接importを解消した。予定自体、配置作業員、通知の作成・更新・削除は、操作後のlocal表示へ直ちに反映してCallableを独立送信し、listenerの正本で表示全体を無条件に置き換える。通常操作用のpending lock、queue、更新順保証、履歴管理は持たず、失敗時だけ対象予定を明示再取得する。client側はdraft・request・local表示の純粋contractとUX用policyに限定し、Functionsを正式な認可・保存境界として維持する。製品clientの直接参照0件、一時Vite設定の撤去を確認した。簡素化後の会社管理者Local UIでは合成予定の作成と作業員のドラッグ配置が成功し、配置人数・作業員タグ・仮配置状態が操作直後に変わり、listener反映後も予定カードと操作buttonを含めて維持された。Dev反映・受入れ、commit、main統合は未実施である。予定更新dialogの削除checkboxと、実績化前だけを削除できる既存Callable境界は維持する。[FUT-0186](../implementation/future-actions.md#fut-0186-配置管理の楽観的更新回帰を復旧する)を正とする。
