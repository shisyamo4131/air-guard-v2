...
4. Siteは[専用ロードマップ](site.md)で管理する。SITE-08では先行の受入れに加え、利用者許可のLocal Chromeで背景trigger同期、各編集、snapshot不変、予定更新・実績化を確認し、旧Site更新と予定プリセットを修正した。最終build・review・source統合まで成功し、生成物cleanup承認待ちのため90%を維持する。SITE-09のDev反映・remote/data確認は別承認である。
