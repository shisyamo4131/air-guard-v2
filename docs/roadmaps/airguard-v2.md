...
5. Employee master改修の現在工程・承認・次作業は[Employeeロードマップ](employee.md)を正とする。利用者へ提示した現行調査に基づく計画と、個別仕様の未決判断を分け、別taskの破棄済み案を流用しない。Siteと進捗・完了条件・承認を合算せず、製品実装・Dev反映へ承認を拡張しない。
