...
3. Outsourcerは[専用ロードマップ](outsourcer.md)で管理する。利用者は、Outsourcerを特定の協力会社masterとし、同じ外注先を配置へ複数回登録できる現行方式を維持して、旧試行の人数集約方式を採用しないと確定した。OUT-01からOUT-05の更新権限・保存data・status・非archive・検索表示に加え、OUT-06で重複配置を安定した`workerId`の別明細として扱うlocal実装・契約検証を完了し、専用ロードマップで80%を加点した。OUT-07以降とDev反映は未承認であり、本ロードマップ本体の進捗には加点しない。
