...
4. Siteは[専用ロードマップ](site.md)で管理する。SITE-08では先行の受入れに加え、利用者許可のLocal Chromeで背景trigger同期、各編集、snapshot不変、予定更新・実績化を確認し、旧Site更新と予定プリセットを修正した。最終build・review・source統合と明示承認後の生成物cleanupを完了し、Siteの進捗を90%から95%へ更新した。SITE-09のDev反映・remote/data確認は別承認である。
5. 次の作業はEmployee master改修のロードマップ作成である。現行仕様、個人情報・権限、UI、client/Functions/RulesのCRUD経路、data契約、User・配置・勤怠等との依存、既知課題、既存testをrepositoryから調査し、サブエージェントによる設計・独立レビューと相互検討を経て利用者へ提示する。[ADR 0055](../decisions/0055-scope-discovery-and-acceptance-review.md)に従い当初目的・対象外・具体的完了条件と証拠・独立問題の後続修正phaseを明確にする。今回の指示はロードマップ作成までであり、実装・Dev反映の承認へ拡張しない。別taskの破棄済みEmployee案を正本・進捗・承認済み計画として扱わず、Siteと進捗・完了条件・承認を合算しない。
