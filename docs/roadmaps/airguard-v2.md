...
3. Outsourcerは[専用ロードマップ](outsourcer.md)で管理する。利用者は、Outsourcerを特定の協力会社masterとし、同じ外注先を配置へ複数回登録できる現行方式を維持して、旧試行の人数集約方式を採用しないと確定した。OUT-01の更新権限と破壊操作停止はlocal実装・検証を開始している。OUT-02以降とDev反映は未承認であり、この時点では進捗加点しない。
