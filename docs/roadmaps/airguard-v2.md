...
4. 次のマスタ管理phaseはSiteとする。現行仕様、UI、client/Functions/RulesのCRUD経路、data契約、関連機能、既知課題、既存testのread-only確認と[Site専用ロードマップ](site.md)の利用者承認を終え、SITE-01を開始した。CONF-0049は誤登録・重複archiveの安全境界、CONF-0050は予定をlive Site、実績を作成時snapshot、確定請求をrevision snapshotとする時点として確定した。次はCONF-0051〜CONF-0053の取極め契約を判断する。製品実装は未承認である。
5. Employee masterはSiteと分離した将来phaseとして残す。別taskで作成されたEmployee改修案は利用者判断により破棄し、正本・進捗・承認済み計画として扱わない。再開する場合は、現行仕様、個人情報・権限、UI、client/Functions/RulesのCRUD経路、data契約、User・配置・勤怠等との依存、既知課題、既存testをrepositoryから改めて確認し、Siteと進捗・完了条件・承認を合算しない。
