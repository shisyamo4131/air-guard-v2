...
| FGA-04-EMPLOYEE-MANAGER-LWW-02 | In progress | Local実装として、一覧作成、基本・国籍・警備員・資格・3保険、両Manager、通常reader／Rulesをtenant共通権限とdocument last-write-winsへ揃え、必須Local検証を完了した。保険は状態遷移条件だけを残し、退職・誤退職訂正・archive・User/Authは専用操作を維持する。旧Callable entrypoint停止、Dev反映・受入れが残るため未完了 |
