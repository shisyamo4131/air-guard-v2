...
- FGA-04-EMPLOYEE-MANAGER-LWW-02の最初のLocal実装では、在職一覧の作成、詳細の基本・国籍・警備員情報を単数／複数形Employee ManagerからEmployee modelの通常`create`／`update`へ接続した。通常Employeeの一覧・詳細・Rulesは、同一tenantの有効な本登録Userへrole非依存で開き、退職後の通常更新、client delete、archive、退職・誤退職訂正、User/Authの専用境界を維持した。公開schema packageにない既存の保険操作世代値は、local Employee schemaで欠落を防ぎ、新規作成時だけ3種を0で初期化する。既存dataの一括変換は行わない。全domain 1,555件、Local Emulator 182件、build、文書検証を通過した。[Local検証記録](../verification/fga-04-employee-manager-lww-local.md)を参照する。資格と保険の画面はまだ旧専用保存・role制限のままで、Dev反映・利用者受入れも未実施のため、このcheckpointと進捗加点は未完了である。
...
| FGA-04-EMPLOYEE-MANAGER-LWW-02 | In progress | 最初のLocal実装として、一覧作成、基本・国籍・警備員編集、単数／複数形Manager、通常reader／Rulesをtenant共通権限とdocument last-write-winsへ揃え、必須Local検証を完了した。退職・誤退職訂正・archive・User/Authは専用操作を維持する。資格・保険画面のrole制限と旧専用保存の整理、Dev反映・受入れが残るため未完了 |
