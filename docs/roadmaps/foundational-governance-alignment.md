...
- 開始日: 2026-09-09
- 現在の進捗: 80%
- 目的: 利用者が提示するプロジェクト固有の根本ルールを正本へ反映し、通常業務CRUDに重なったrole・permission制御、専用Callable、重複Rulesを可能な限り緩和・簡素化する。通常CRUDはDomain Manager、`AirItemManager`／`AirArrayManager`、FireModel／ClientAdapterの標準保存を有効活用し、確認済み例外と技術的にserverを必要とする最小operationだけを専用境界へ残す。
...
- FGA-06-TRANSACTION-SIMPLIFICATION-SCOPE-03では、FGA-06を未改修CRUDの追加やCallable内の権限調整ではなく、transaction系に残る通常CRUDのDomain Manager・標準client保存への移行、Rules簡素化、Callable撤去・最小化を完了する工程として再確認した。[ADR 0071](../decisions/0071-normal-business-manager-and-callable-boundary.md)によりAir Managerの適用範囲を通常業務data全般へ拡張し、[現行棚卸し](../implementation/operation-crud-simplification-inventory.md)で`saveOperation`の操作を通常CRUD候補、確認済み例外候補、技術要件候補、仕様未決へ分けた。稼働実績詳細の稼働外売上は操作可能な現行画面を維持し、権限仕様を未決とした。このcheckpointは文書・分類のみで製品code、Rules、Functions、data、Dev・Prodを変更せず、進捗は80%を維持する。
- FGA-06-RESULT-MANAGER-CLIENT-04は固定製品commit `40316475`で、非lock実績の基本情報を`AirItemManager`、従業員・外注先明細を`AirArrayManager`へ接続し、標準client document保存へ移した。Rulesは同一tenantの有効な本登録Userによる既存updateだけを開き、create、delete、lock、稼働外売上、請求調整、lifecycle IDを保護する。稼働外売上と物理削除は従来画面・Callableを維持し、旧`overview`・`workers` Callable分岐もrollback用に残した。全domain 1454件、Local Emulator 180件、固定commitの専用UI buildは成功したが、Dev反映・ブラウザ実操作・見た目受入れ前のためcheckpointはLocal completed、phase進捗は80%を維持する。[Local検証記録](../verification/fga-06-result-manager-client-local.md)を参照する。
...
| FGA-06-RESULT-MANAGER-CLIENT-04 | Local completed | 非lock実績の基本情報・従業員／外注先明細をAir Managerと標準client保存へ移し、Rulesをtenant共通の限定updateへ簡素化した。旧画面構成、稼働外売上、物理削除、旧Callable rollback経路を維持し、自動検証と固定commitの専用UI buildを完了した。Dev反映・ブラウザ操作・見た目受入れ後に完了判定する |
