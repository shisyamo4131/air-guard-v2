...
| FGA-06-SCHEDULE-MANAGER-RESTORE-09 | In progress | 予定の単数・複数Manager、配置作業員、複製、配置通知をAir Manager／model保存へ戻し、予定・通知Rulesをtenant境界へ簡素化した。release `21013672`のDev受入れで、上下番確定の左右画面がManagerの重複した`table`表示口に上書きされる不具合を確認した。汎用転送から`table`を除外するLocal補正、回帰test、全domain 1,434件、buildは完了したが、補正版のcommit・Dev再反映と、配置管理・上下番確定の再受入れが残るため完了・加点しない。[Local検証記録](../verification/fga-06-schedule-manager-restore-local.md)と[Dev反映・受入れ記録](../verification/fga-06-schedule-manager-restore-dev.md)を参照 |
