...
- FGA-03-SITE-MANAGER-LWW-05では、Site通常CREATE・UPDATEを単数／複数形ManagerからSite modelの標準`create`／`update`へ直接接続し、listener由来instanceのdocument単位last-write-winsへ揃えた。作成は履歴で確認した3ステップ入力を維持し、Customer未登録でも取引先名だけで仮登録できる。通常保存用の`useSiteActions`、専用writer、埋込みCustomerのexact 6-field要件は撤去し、終了・再有効化・archive・Agreementは専用操作を維持する。既存dataの一括変換、schema、packageは変更しない。commit `7a13d151`をGitHub Actions #13でFirestore・Functions・HostingへDev反映し、Codex専用合成tenantでCustomer未登録CREATE、通常UPDATE、終了、再有効化、archive、Agreement追加・再表示、Agreement保持Siteのarchiveまで成功した。[Local検証記録](../verification/fga-03-site-manager-lww-local.md)と[Dev受入れ記録](../verification/fga-03-site-manager-lww-dev.md)を参照する。別actorと利用者の主観受入れが未確認のため進捗は28%のままとする。tenant拒否とapplicationを介さないrequestは今回想定しない範囲である。
...
| FGA-03-SITE-MANAGER-LWW-05 | Dev technically verified | Site通常CREATE・UPDATEを`SiteManager`／`SitesManager`からSite modelへ直接接続し、通常保存用`useSiteActions`と専用writerを撤去した。3ステップ作成とCustomer未登録時の仮登録を維持し、専用Agreement・終了・再有効化・archiveを分離する。domain 1,549件、Local Emulator 182件、GitHub Actions #13、Codex専用合成tenantのCREATE・UPDATE・終了・再有効化・archive・Agreement追加／再表示を完了した。別actorと利用者の主観受入れが未確認のためFGA-03進捗は未加点。tenant拒否とapplicationを介さないrequestは今回想定しない範囲 |
