...
| FGA-03-SITE-MANAGER-LWW-05 | Dev technically verified | Site通常CREATE・UPDATEを`SiteManager`／`SitesManager`からSite modelへ直接接続し、通常保存用`useSiteActions`と専用writerを撤去した。3ステップ作成とCustomer未登録時の仮登録を維持し、専用Agreement・終了・再有効化・archiveを分離する。domain 1,549件、Local Emulator 182件、GitHub Actions #13、Codex専用合成tenantのCREATE・UPDATE・終了・再有効化・archiveを完了した。Agreement、別actor、tenant拒否と利用者の主観受入れが未確認のためFGA-03進捗は未加点 |
