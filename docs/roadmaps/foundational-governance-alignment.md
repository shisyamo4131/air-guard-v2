...
- 利用者は[Domain Manager wrapperとdata編集dialog規約](../decisions/0068-domain-manager-wrapper-and-editor-dialog-convention.md)を採用した。Customer、Site、Employee、Outsourcerの提供済み通常C/U/Dは単数・複数形のdomain Managerを文脈別入口とし、Autocomplete内createも複数形Managerへ接続する。通常data編集dialogは最大幅480pxを既定とし、archive・復旧・物理削除等の例外はgeneric deleteへ接続しない。製品実装はCustomerから小checkpointで移行する。
- 利用者はFGA-02-RULES-01のDev releaseを完了し、FGA-02-CUSTOMER-UPDATE-01の固定local commit、続くManagerガバナンスとCustomer追加改修、利用者Local確認を承認した。現在のCustomer Update実装は単数`CustomerManager`の基礎として維持し、同Managerへ利用されないCreateは追加しない。collection・一覧・選択文脈のCreate、複数形`CustomersManager`、一覧・Autocomplete接続、archive入口の責務、480px幅、およびこれら追加改修後の利用者Local受入れは未完了である。FGA-02の得点は、追加改修、必要な固定commit、Dev反映・受入れが未完了のため0のままとする。

## FGA-02 Customer内部checkpoint

| Checkpoint | 状態 | 範囲・完了条件 |
|---|---|---|
| FGA-02-MANAGER-GOVERNANCE-01 | In progress | domain Manager、Autocomplete create、例外operation、480px既定をproject rule・仕様・ADR・roadmapへ反映し、文書・governance検証を完了する |
| FGA-02-CUSTOMER-MANAGER-02 | Not started | 既存Updateを担う単数`CustomerManager`を通常編集dialogの480px既定へ整合する。利用されないCreateは追加せず、document全体LWW、schema、listener、error・loadingを維持する |
| FGA-02-CUSTOMERS-MANAGER-03 | Not started | `AirArrayManager`をラップする`CustomersManager`を新設し、Customer一覧とAutocomplete内createを接続する。commit成功と割当済みdocument IDを持つ作成結果だけを選択し、list listenerを不必要に待たない。その後、主対象listはlistenerへ収束し、従属先の名称・詳細だけはtenant scopeを分離した`useFetch`共有cacheを優先できる |
| FGA-02-CUSTOMER-ARCHIVE-04 | Not started | Customerの削除相当UI入口をdomain Managerの責務へ整理し、既存archive専用Callable、従属検査、監査、冪等性を維持する。generic client deleteと物理削除は開放しない |
| FGA-02-CUSTOMER-LOCAL-05 | Not started | 会社管理者の利用者Local環境でCreate・Update・一覧・Autocomplete・archive入口と480px幅を確認し、失敗経路・listener収束・例外境界を記録する |
