...
- FGA-06-RESULT-EDIT-NORMAL-AUTH-02では、未確定かつlockされていない現場稼働実績の基本情報と従業員・外注先明細の編集を、同じtenantの有効な本登録Userへrole非依存で許可するFunctions実装と自動検証をLocal sourceへ反映した。実績の作成・複製・削除、予定からの確定、配置通知、稼働外売上、請求操作のactor条件は変更していない。`saveOperation` Callable、最新状態との照合、lock拒否、transactionと関連document更新経路も維持した。[実装記録](../implementation/operation-result-edit-authorization.md)と[Local検証記録](../verification/fga-06-result-edit-normal-auth-local.md)を参照する。Dev反映・受入れは未実施であり、checkpointとFGA-06の加点は行わずphase全体は80%を維持する。
