...
| FGA-06-SCHEDULE-MANAGER-RESTORE-09 | In progress | 予定の単数・複数Manager、配置作業員、複製、配置通知をAir Manager／model保存へ戻し、予定・通知Rulesをtenant境界へ簡素化した。全domain、Local Emulator、固定commitのUI buildを通過し、release `21013672`をFirestore・HostingへDev反映した。正規画面受入れと、利用者報告の配置管理・上下番確定エラーの調査が残るため完了・加点しない。[Local検証記録](../verification/fga-06-schedule-manager-restore-local.md)と[Dev反映記録](../verification/fga-06-schedule-manager-restore-dev.md)を参照 |
