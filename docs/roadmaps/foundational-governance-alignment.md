...
| FGA-06 その他transaction系機能 | 20 | 0 | In progress | master完了後、残るtransactionの通常CRUDをDomain Manager・標準client保存へ寄せ、operationごとにRules簡素化、Callableの撤去・最小化、例外、data互換を小checkpointで整合し、Dev受入れとphase closeoutを完了する |
...
- FGA-02-MANAGER-GOVERNANCE-01はcommit `25069a79e398ad3fa98b960fb758f18f053238e0`で一度完了したが、単数・複数形Managerを画面の選択文脈で分類した判断に誤りがあり、[ADR 0069](../decisions/0069-domain-manager-editable-state-ownership.md)とgovernance correction commit `b89e5c86`で訂正した。現行基準はeditable stateの所有単位であり、単数Managerは外部既存instanceまたはその場の新規instance、複数形Managerは配列と行選択dispatchを所有する。選択後はAirArrayManagerの`beforeEdit`により内部editorまたは詳細navigationを選び、相互にManagerを内包しない。FGA-02ではFirestore上の通常のmaster dataへ適用し、4機能を最初の適用例とした。[ADR 0071](../decisions/0071-normal-business-manager-and-callable-boundary.md)により現行の適用範囲をtransactionを含む通常業務data全般へ拡張する。480px、archive等の例外、listener・cache、server認可境界は変更しない。
...
- FGA-06-SCHEDULE-NORMAL-AUTH-01では、現場稼働予定の作成・複製・基本情報変更・配置作業員変更・表示順変更・削除を通常業務としてtenant共通のserver権限へ揃えた。関連documentを同時に整える`saveOperation` Callable、Site revision、通知取消し、実績化済み予定の変更・削除拒否は維持し、配置通知作成、稼働実績への確定、稼働実績・請求のactor条件は変更していない。製品commit `b478e99e`をrelease commit `d8d3c7c7`のGitHub Actions #29でFunctionsへDev反映し、Codex専用合成tenantの認証済みChromeで予定の作成・更新・複製・削除とcleanupを確認した。[Local検証記録](../verification/fga-06-schedule-normal-auth-local.md)と[Dev受入れ記録](../verification/fga-06-schedule-normal-auth-dev.md)を参照する。予定入力内の`現場を新規登録`不具合は2026-09-14にLocal修正し、[FUT-0190](../implementation/future-actions.md#fut-0190-現場稼働予定入力内の現場新規登録を復旧する)をResolvedとした。Dev再反映・受入れは未実施である。FGA-06全体には残るtransaction系機能があるため、このcheckpoint完了だけでは20点を加点せず、phase全体は80%を維持する。
- FGA-06-RESULT-EDIT-NORMAL-AUTH-02では、未確定かつlockされていない現場稼働実績の基本情報と従業員・外注先明細の編集を、同じtenantの有効な本登録Userへrole非依存で許可するFunctions実装と自動検証を完了した。実績の作成・複製・削除、予定からの確定、配置通知、稼働外売上、請求操作のactor条件は変更していない。`saveOperation` Callable、最新状態との照合、lock拒否、transactionと関連document更新経路も維持した。製品commit `d759409c`をrelease commit `4bd80e3f`のGitHub Actions #30でFunctionsへDev反映し、Codex専用合成tenantの認証済みChromeで実績の作成、基本情報の備考変更、再表示、実績削除、合成Site archiveまで確認した。別actor、従業員・外注先明細編集、拒否経路はDevで再実行せずLocal自動検証を正とする。[実装記録](../implementation/operation-result-edit-authorization.md)、[Local検証記録](../verification/fga-06-result-edit-normal-auth-local.md)、[Dev受入れ記録](../verification/fga-06-result-edit-normal-auth-dev.md)を参照する。FGA-06全体には残るtransaction系機能があるため、このcheckpoint完了だけでは20点を加点せずphase全体は80%を維持する。
- FGA-06-TRANSACTION-SIMPLIFICATION-SCOPE-03では、FGA-06を未改修CRUDの追加やCallable内の権限調整ではなく、transaction系に残る通常CRUDのDomain Manager・標準client保存への移行、Rules簡素化、Callable撤去・最小化を完了する工程として再確認した。[ADR 0071](../decisions/0071-normal-business-manager-and-callable-boundary.md)によりAir Managerの適用範囲を通常業務data全般へ拡張し、[現行棚卸し](../implementation/operation-crud-simplification-inventory.md)で`saveOperation`の操作を通常CRUD候補、例外・技術要件候補、仕様未決へ分けた。その後[ADR 0072](../decisions/0072-transaction-delete-client-trigger-boundary.md)でtransaction物理削除をclient・Trigger境界へ確定した。稼働実績詳細の稼働外売上は操作可能な現行画面を維持し、権限仕様を未決とした。このcheckpointは文書・分類のみで製品code、Rules、Functions、data、Dev・Prodを変更せず、進捗は80%を維持する。
...
- FGA-06-RESULT-DELETE-CLIENT-06は固定製品commit `07511fb3`で、稼働実績詳細の既存削除dialogを`OperationResultManager`と標準client deleteへ接続し、Rulesをtenant共通の非lock deleteへ開いた。旧`saveOperation`の実績delete入力は拒否し、関連data連携は既存Triggerへ維持した。Localでは従業員10名を含む削除、Devでは作業員CRUD後の合成実績削除、一覧0件、Trigger errorなしを確認した。[Local検証記録](../verification/fga-06-result-delete-client-local.md)と[Dev受入れ記録](../verification/fga-06-result-callable-restore-dev.md)を根拠にcheckpointを完了した。FGA-06全体の残作業があるためphase進捗は80%を維持する。
- FGA-06-RESULT-CALLABLE-RESTORE-07は固定製品commit `5dac8d49`で、過去の`WorkersManager`／`AirArrayManager`と親`OperationResult.update()`の接続を基準に、後から追加された`useOperationResultWriter`、Employee存在確認transaction、再読込handler、実績`overview`／`workers` Callable入力を撤去し、OperationResult Rulesから通常CRUの従属先存在確認を外した。初回Dev受入れで発見した作業員初期値継承不足は、過去repositoryと現行component契約に合わせたcommit `8d90d5d1`で補正した。補正後に会社管理者sessionで作業員追加・更新・削除・再読込、実績物理削除、Trigger errorなし、既存UIを確認した。未決の稼働外売上、請求field、lock、実績作成・複製は変更していない。[Local検証記録](../verification/fga-06-result-callable-restore-local.md)と[Dev受入れ記録](../verification/fga-06-result-callable-restore-dev.md)を根拠にcheckpointを完了した。FGA-06全体の残作業があるためphase進捗は80%を維持する。
...
| FGA-06-TRANSACTION-SIMPLIFICATION-SCOPE-03 | Completed | 通常transaction CRUDにもAir Manager・標準client保存を適用し、Rulesを簡素化してCallableを例外・具体的技術要件の最小範囲へ限定する目的を[ADR 0071](../decisions/0071-normal-business-manager-and-callable-boundary.md)へ確定した。現行`saveOperation`を[操作別に棚卸し](../implementation/operation-crud-simplification-inventory.md)し、稼働外売上は操作可能な現状維持・権限未決とした。製品変更・進捗加点なし |
...
| FGA-06-SCHEDULE-MANAGER-RESTORE-09 | In progress | 予定の単数・複数Manager、配置作業員、複製、配置通知をAir Manager／model保存へ戻し、予定・通知Rulesをtenant境界へ簡素化した。release `21013672`のDev受入れで確認した上下番確定の左右画面上書きを補正し、回帰test、全domain 1,434件、buildを通過したcommit `12f05e5a`をGitHub ActionsでHostingへ再反映した。SCR-02と重複する上下番確定処理画面の左右独立scroll・固定操作・確定と再表示は2026-09-17の利用者報告で受入れ済みとする。一方、配置管理など別範囲の受入れ・残存riskがあるためFGA-06全体は完了・加点しない。[Local検証記録](../verification/fga-06-schedule-manager-restore-local.md)と[Dev反映・受入れ記録](../verification/fga-06-schedule-manager-restore-dev.md)を参照 |
| FGA-06-TRANSACTION-PARENT-INDEPENDENCE-10 | In progress | [ADR 0074](../decisions/0074-transaction-parent-reference-independence.md)により、トランザクション文書の親Customer／Site／Employee存在barrier、archive時のtransaction走査、集約Employee検索索引を撤去する。請求失敗が勤怠等を止めないprojection間分離と、[Dev復旧計画](../implementation/operation-result-projection-recovery.md)をLocalで実装中。Dev反映、実data dry-run／repair、利用者受入れは未実施で、進捗は80%のままとする |
| FGA-06-BILLING-PAYMENT-DATE-11 | Completed | [SCR-01](standard-crud-alignment.md#scr-01-入金予定日編集の内訳)で請求の入金予定日編集を単数ManagerとBilling標準updateへ接続し、Rules簡素化、旧専用Callable撤去、Local自動検証、GitHub ActionsによるDev反映、会社管理者による変更・解除・再表示・既存表示維持の受入れを完了した。失敗経路は自動test成功を採用し、backend停止時の画面確認は利用者判断で完了条件外とした。[Dev受入れ記録](../verification/scr-01-billing-payment-date-dev.md)を参照。FGA-06全体には残作業があるためphase進捗は80%を維持する |
