...
- 現在の進捗: 100%
- 進捗の解釈: EMP-08初回reviewで90%から40%へ訂正した後、EMP-02・05・06を是正・再受入れして90%へ戻し、EMP-08の5点を加えて95%とした。2026-09-07にEMP-09のbounded Dev releaseと権限別受入れを完了して100%とした。部分加点はない。[EMP-08記録](../verification/employee-08-local.md)と[Dev受入れ結果](../implementation/master-dev-acceptance-plan.md#no10後のcustomeroutsourceremployee-dev受入れ結果)を参照する。
...
EMP-05計画・初回終了時点の履歴: 内部順序はreader/参照契約→参照writer・index保持→旧trigger/再生成対処→専用archive→競合/表示回帰。重み20は内部で部分加点せず、全受入れ完了後に加点した。EMP-05完了報告で停止し、EMP-06は別の開始指示を待つ。
...
|---|---:|---:|---|---|
| EMP-01 契約・対象確定 | 10 | 10 | Completed | 2026-09-07に採用済み仕様・対象・工程境界・受入条件を再照合し、業務判断を要する矛盾なしとして再受入れた。actor・保存/競合・住所/座標・archive形式/従属/閲覧・表示名/作成導線・段階移行/提供工程、EMP-02へ渡す入力・保存・test前提を維持する。製品runtimeや後続工程の成功を意味しない。 |
| EMP-02 作成・基本・国籍 | 20 | 20 | Completed | EMP-08で保存待機中の変更通知と拒否後の再読込条件を是正・再受入れした。以下の成果と[最終検証](../verification/employee-08-local.md)を照合済み。 専用保存・独立draft・7actor read/直接CUD拒否、住所と派生field・局所競合・異field保持を実装済み。既存の実UI/backend・自動検証に、UI復元後の一覧・登録dialog・詳細表示と最終clean source専用buildを追加照合し、2026-09-07に再受入れた。[検証記録](../verification/employee-02-04-local.md)を参照。 |
| EMP-03 警備員登録・資格 | 10 | 10 | Completed | 専用保存・原配列位置・raw期待値、同名資格・名称変更・対象行削除・2画面競合/再選択、警備登録/解除9field・非対象保持を実装・検証済み。資格dialogの復旧と最終clean source専用buildを追加照合し、2026-09-07に再受入れた。[検証記録](../verification/employee-02-04-local.md)を参照。 |
...
| EMP-05 閲覧互換・参照保護・アーカイブ | 20 | 20 | Completed | EMP-08で認可購読の取得失敗と正常な空表示を分離して再受入れした。以下の成果と[最終検証](../verification/employee-08-local.md)を照合済み。 7actorの原本/archive read、現在認可と期間cache、参照writer/Rules・背景索引・限定整合確認・旧削除trigger無作用・専用archiveを実装・検証済み。UI回帰修正後に作業員と稼働外売上の既存行表示・編集復元・取消を利用者Chromeで確認し、2026-09-07に再受入れた。[最終matrixと証拠](../verification/employee-05-local.md#05-e-統合次工程review)を参照。通常APIのlocal接続はDev反映前準備で行い、remote公開・対象tenant開放・実data整合/補完・Dev受入れはEMP-09の別承認とする。 |
...
会社管理者では合成Employeeの通常作成、基本情報・国籍・警備員登録・資格・3保険の保存、退職と誤退職訂正による復帰を確認した。経理accountでは在職一覧と詳細を閲覧でき、作成・編集・退職・User登録・archiveの導線が表示されないことを確認した。合成Employeeは在職状態でDevに保持し、実Employee、User／Authentication、予約・通知・請求、外部providerは操作していない。通常archive APIのtenant allowlistは空であり、会社管理者に表示されるarchive確認dialogを開いて閉じるまでに留め、archive要求は送信していない。この完了によりEMP-09へ5点を加え、進捗95%から100%とした。
...
1. EMP-UI-R1の一覧flex・dialog固定・作業員/稼働外売上明細と、EMP-INS-R1の保険項目不存在の修正は[検証記録](../verification/employee-05-local.md)を参照する。直接test・専用Emulator・利用者Chrome・clean source専用buildまで確認済み。コンソール問題は申告撤回済みで修正対象に含めない。
2. 2026年6月の利用者Local実績明細で作業員6行と既存行の編集復元・取消、利用者が追加した稼働外売上1行の全列・合計・編集復元・取消を確認した。Codexは保存・追加・削除を行っていない。
