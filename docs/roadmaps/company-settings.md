...
>
> 旧進捗10%はADR 0025に基づく設計成果であり、ADR 0031によるCCB restart後のactive進捗へ持ち越さない。本書は旧CCB成果物のrollback inventoryと判断履歴として保持する。現在の作業は[ロードマップ索引](README.md)から選ぶ。
>
...
- 状態: In progress（仕様確定・compatible reader・Codex専用migration Emulator経路・pre-containment Rules候補をlocal実装済み。既存CRUD互換化をRules deployより先に行う計画へ訂正）
- 現在の進捗: 10%
- 基準日: 2026-08-29
...
- CCB-02 rollout訂正: 2026-08-29承認。localのpre-containment Rules候補はdeploy readinessではない。現行Rules下でCompany clone/runtime state、CompanyManagerのoperation別handler、profile/billing/operations/arrangementのmarker-aware Client/Server writer、signup・agreements・site/schedule order等のcallerを移行する。LEGACYはscope別expected-value一致時のroot partial updateだけ、STAGEDはmaintenance中の通常write/signup拒否、ACTIVEはSettings＋必要auditとし、dual-writeしない。現行・候補Rules両回帰と旧writer 0件を確認してからRulesを閉じ、deny receipt後に同じmaintenance内でstagingとactivationを完了する。
...
| 編集draft | AirItemManagerがCompany cloneを編集し、`updateProperties`はlocal draftの既存top-level propertyだけを変更 | Firestore部分更新ではなくchanged-key追跡もない。Company cloneがnon-enumerableなCCB runtime stateを失うため、clone補正とapplication-owned operation別handlerが未実装 |
...
additive schemas、Admin SDKの旧破壊操作fail-closed、承認済みCCB backup方針、compatible readerまでは準備済みである。次は現行Rulesを維持したまま、Company cloneがtrusted CCB runtime stateを非永続のまま保持できるようにし、CompanyManagerとagreements・site/schedule order等をapplication-ownedのoperation別handlerへ移す。同じ4 Callableをmarker-awareにし、LEGACYではscope別expected-value一致時だけ既知root fieldをpartial updateしてSettings/audit writeを0、STAGEDでは通常write/signupを拒否、ACTIVEではSettingsと必要auditだけを更新する。現行Rulesと候補Rulesで同じ回帰matrixを通し、対象環境で既存CRUD継続と旧whole-document writer 0件を確認した後にmaintenanceを開始して通常write/signupを停止し、pre-containment Rulesをdeployする。receipt確認後にcomplete Settingsをcreate-only stagingして同じmaintenance内でactivationする。旧field cleanupは別migrationとする。関連Schemas/Admin SDK repository変更は対象、互換性、公開・導入順、rollbackを示して別承認を得る。
...
| CCB-02 data・field・package・既存CRUD互換契約 | 10 | 0 | In progress | Dev fixtureと全Company callerを再照合し、root・各Settingsのexact schema、default、unknown/legacy field、Schemas/client/Functions/Admin SDKのrelease順、dual-read期間、migration mappingを検証する。Schemas公開、3 consumerのexact導入、Admin SDK安全停止、client compatible reader、canonical parity・backup coverage・SettingAudits restore契約、pure migration/audit planと合成回帰、Codex専用REST create-only transaction/post-check、pre-containment Rules候補のlocal回帰は完了した。Company clone/runtime state、実CompanyManager submit、LEGACY/STAGED/ACTIVE marker-aware writer、全既存caller、現行・候補Rules両回帰、対象環境の旧writer 0件、deployed Rules receipt、maintenance内staging/activation検証が残る。 |
...
- UI/adapter: 実CompanyManager mount・submit、AirItemManagerのloading/error/dialog維持、Company cloneのLEGACY/ACTIVE/ERROR state非永続保持、scope別exact payload、`draft.update()`・root whole-set不在、invoiceNumberのbilling scope、`attendanceManagementMode`からcanonical fieldへのmapping。
- rollout: 現行Rulesと候補Rulesの同一matrix、Client/Server先行release、既存CRUD継続、旧writer 0件、pre-containment deployed receipt、receipt後だけのSettings staging、Rules閉鎖後のcompatible rollback。
- transition writer: LEGACYのscope別expected-value一致・exact partial update、reserved field/root全体/Settings/audit write 0、STAGEDの通常write/signup拒否、ACTIVEのexpectedRevision・Settings path・audit atomicity、maintenance中のstagingからactivation、dual-write 0。
