...
| 2026-08-28 | 10% | ±0 | CCB local-only pure migration planner、Firestore REST Valueのtype-tagged canonical digest、合成fixture、17件の回帰testを実装した。Schemas exact `.167` mappingを使い、8 target create-only、complete exact、partial/不一致/unknown/invalid/ambiguous/orphan/edition未確認の全体停止、manifest digest binding、fresh re-planを確認した。Firestore reader/apply、Rules、backup/audit restore、Dev stagingが残るため、部分加点なしのCCB-02は未完了とする。 |
