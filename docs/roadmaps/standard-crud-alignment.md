...
| SCR-02 配置通知の状態更新・編集 | 10 | 0 | In Progress（Local補正確認済み・Dev反映待ち） | Managerと本人向け確認・上番・下番を標準クラスへ接続し、Schemas生成とのnotify parityを揃え、Local補正で左Listと右Detail本文の独立scroll、右側操作部の固定、確定操作への到達を利用者確認済み。補正をDevへ反映してDev受入れを行う |
...
補正は`components/OperationResult/Generator/index.vue`のinner flex rowへ`overflow-hidden`と`min-height:0`を追加して完了した。完成条件は、短いviewport、alert有無、schedule未選択／選択後、reload表示、worker／security report等の縦長内容で、header/reloadと確定buttonが常時到達可能、ListとDetail本文が独立してscrollし、狭い横幅の既存挙動を維持すること。画面全体scrollの単純復活だけでは完了としない。source regression testとreviewは完了し、Local UIは利用者確認により合格した。Dev再反映・Dev再受入れは未実施のためSCR-02を0点のまま保持する。
