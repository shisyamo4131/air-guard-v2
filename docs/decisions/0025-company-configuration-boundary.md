...
実装前にDevのFirestore editionを再確認し、`STANDARD / FIRESTORE_NATIVE`の現在baselineと一致することを確かめる。互換readerを先に導入し、設定documentのcreate-only backfill、dual-read検証、操作別writer、制限的Rules、旧全体writer撤去の順で進める。dual-writeを採用する場合は期間・正本・競合判定をcheckpointで固定し、無期限に残さない。

migrationは対象company、field mapping、write件数、plan digest、backup、rollback、停止条件を固定する。既存rootやlegacy fieldを最初のreleaseで削除せず、旧clientを戻せる互換期間を設ける。rollbackは新setting documentを推測削除せず、既知の互換releaseへ戻し、正本切替後のdataを再dry-runして別repairで扱う。
