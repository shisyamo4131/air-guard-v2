<script setup>
/*****************************************************************************
 * @file ./components/Users/Iterator/index.vue
 * @description ユーザー情報表示用データイテレーターコンポーネント
 * @author shisyamo4131
 ...
 * @emits click:detail - 詳細ボタンがクリックされたときに発火するイベント。引数は詳細対象のユーザー情報オブジェクト。
 * @emits click:edit - 編集ボタンがクリックされたときに発火するイベント。引数は編集対象のユーザー情報オブジェクト。
 ...
 *
 * @slot default - ユーザー情報オブジェクト群を表示するためのスロット。
 *  @see https://v3.vuetifyjs.com/en/api/v-data-iterator/#slots-default
 *
 * @slot item - 各ユーザー情報アイテムの表示をカスタマイズするためのスロット。スロットプロパティには、ユーザー情報オブジェクト、選択状態、編集・展開・選択機能の表示有無などが含まれます。
 *  @property {Object} customer - アイテムに対応するユーザー情報オブジェクト
...
defineOptions({ name: "UsersIterator" });
...
<template>
  ...
  >
    ...
    <template #item="{ item, isSelected, select, showSelect }">
      ...
      >
        <template #append>
          <slot name="card-append" v-bind="{ item: item.raw }" />
        </template>
      </UserCard>
    </template>
