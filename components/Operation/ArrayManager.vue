...
const props = defineProps({ kind: { type: String, required: true }, docs: { type: Array, default: () => [] }, customInput: { type: Object, default: null }, label: { type: String, default: "稼働情報" }, beforeEdit: { type: Function, default: null }, handleClickUpdate: { type: Function, default: null }, tableProps: { type: Object, default: () => ({}) } });
...
const table = computed(() => ({ ...props.tableProps, items: props.docs, label: props.label, itemKey: "docId", disabled: editor.busy.value, isLoading: editor.busy.value, loading: editor.busy.value, toCreate, toUpdate, toDelete, "onClick:create": toCreate, "onClick:update": props.handleClickUpdate || toUpdate, "onClick:delete": toDelete }));
