<script setup>
/*****************************************************************************
 * @file ./components/Customers/Iterator/index.vue
 * @description 取引先情報表示用データイテレーターコンポーネント
 * @author shisyamo4131
 * @extends AirDataIterator
 *
 * @property {Array} customers - 表示する取引先情報の配列
 ...
 * @emits click:detail - 詳細ボタンがクリックされたときに発火するイベント。引数は詳細対象の取引先情報オブジェクト。
 * @emits click:edit - 編集ボタンがクリックされたときに発火するイベント。引数は編集対象の取引先情報オブジェクト。
 ...
 *
 * @slot default - 取引先情報オブジェクト群を表示するためのスロット。
 *  @see https://v3.vuetifyjs.com/en/api/v-data-iterator/#slots-default
 *
 * @slot item - 各取引先情報アイテムの表示をカスタマイズするためのスロット。スロットプロパティには、取引先情報オブジェクト、選択状態、編集・展開・選択機能の表示有無などが含まれます。
 *  @property {Object} customer - アイテムに対応する取引先情報オブジェクト
...
defineOptions({ name: "CustomersIterator" });
...
const props = useDefaults(_props, "CustomersIterator");
const emit = defineEmits(["click:create", "click:detail", "click:edit"]);
</script>

<template>
  <air-data-iterator item-value="docId" :items="props.customers">
    <!-- HEADER -->
    ...
    <template #no-data>
      <v-card>
        ...
        >
          <template #text>
            <div>指定された条件に該当する取引先情報は登録されていません。</div>
          </template>
        </v-empty-state>
      </v-card>
    </template>
