<script setup>
/*****************************************************************************
 * @file ./components/Outsourcers/Iterator/index.vue
 * @description 外注先情報表示用データイテレーターコンポーネント
 * @author shisyamo4131
 * @extends AirDataIterator
 *
 * @property {Array} outsourcers - 表示する外注先情報の配列
 ...
 * @emits click:detail - 詳細ボタンがクリックされたときに発火するイベント。引数は詳細対象の外注先情報オブジェクト。
 * @emits click:edit - 編集ボタンがクリックされたときに発火するイベント。引数は編集対象の外注先情報オブジェクト。
 ...
 *
 * @slot default - 外注先情報オブジェクト群を表示するためのスロット。
 *  @see https://v3.vuetifyjs.com/en/api/v-data-iterator/#slots-default
 *
 * @slot item - 各外注先情報アイテムの表示をカスタマイズするためのスロット。スロットプロパティには、外注先情報オブジェクト、選択状態、編集・展開・選択機能の表示有無などが含まれます。
 *  @property {Object} outsourcer - アイテムに対応する外注先情報オブジェクト
...
defineOptions({ name: "OutsourcersIterator" });
...
const props = useDefaults(_props, "OutsourcersIterator");
  ...
  <air-data-iterator item-value="docId" :items="props.outsourcers">
    ...
    <template #no-data>
      <v-card>
        ...
        >
          <template #text>
            <div>指定された条件に該当する外注先情報は登録されていません。</div>
          </template>
        </v-empty-state>
      </v-card>
    </template>
