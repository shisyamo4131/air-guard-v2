...
- OperationResult クラスに billingMonth プロパティを実装した。
  - 稼働実績を手動で作成した場合の挙動を確認後、必要があれば修正。
    ...
    - 取極めが決まらない状態での応急処置的な処理（仕様）が必要になるか？となれば現場稼働予定から稼働実績を取り込むときの処理にも影響する。
      - 想定通り、問題になった。
      - Agreement プロパティを保有して、空登録を可能にする。
      - hasAgreement プロパティ（ゲッター）で取極めの設定がない稼働実績を判断できるように。
      - hasAgreement === false のドキュメントは Billing に影響させない。
      - 後で現場に Agreement が登録された後で、Agreement を選択できるようにする。
      - 稼働実績の取り込み処理の際も取極めがなくとも取込ができるようにする。
      - ダッシュボードなどで取極めが設定されていない稼働実績が分かるように。
