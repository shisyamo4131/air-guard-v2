...
import { operationAllowed } from "@/functions/shared/operationWriteContract.js";
import { confirmTerminatedScheduleSite } from "@/composables/application/siteOperationSchedule/confirmTerminatedSite";

export function useOperationSubmission({ billing = false } = {}) {
  ...
  const allowed = computed(() => auth.isSuperUserClaimValid === true && operationAllowed({ uid: auth.uid, companyId: auth.companyId, isSuperUser: auth.isSuperUser }, auth.user?.toObject?.() || auth.user, billing));
  function clearConfirmations() { confirmationSignature = null; confirmations.clear(); }
  function reset() { generation++; busy.value = false; uncertain.value = false; message.value = ""; clearConfirmations(); }
  ...
  async function submit(operations) {
    if (busy.value || uncertain.value || !allowed.value) return false;
    ...
    try {
      ...
      for (const command of operations) if (command.kind === "schedule" && ["create", "duplicate", "overview"].includes(command.action)
        ...
        for (const id of ids) {
          ...
          if (siteStatuses.get(id) === "TERMINATED" && id === (command.changes.siteId || current?.siteId) && !confirmations.has(id)) {
            const site = await read("Sites", id);
            if (!await confirmTerminatedScheduleSite({ siteId: id, site })) { clearConfirmations(); throw new Error("保存を取り消しました。"); }
            verify(owner, ticket); confirmations.add(id);
          }
          command.siteStatuses[id] = siteStatuses.get(id);
        }
      }
      verify(owner, ticket); sent = true;
