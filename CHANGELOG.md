...
- Customer、Site、Employee、Outsourcerの通常C/U/Dを利用文脈別のdomain Manager経由とし、単一documentは`AirItemManager`、collection・一覧・選択は`AirArrayManager`のwrapperへ統一するガバナンスを採用した。Customerでは単数ManagerのUpdateと複数形Managerの一覧・Autocomplete Create接続、480px、archive専用入口を未commit worktreeへ実装した。会社管理者の利用者Localで一覧Create、listener反映、詳細Update、約476pxの通常編集dialog、archive入口・取消、generic物理delete入口なしを確認した。Autocomplete runtime、失敗経路、archive実行、Devは未確認である。[判断](docs/decisions/0068-domain-manager-wrapper-and-editor-dialog-convention.md)、[実測記録](docs/verification/fga-02-customer-manager-user-local.md)、[ロードマップ](docs/roadmaps/foundational-governance-alignment.md)を参照。
