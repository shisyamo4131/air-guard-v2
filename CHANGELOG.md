...
- Domain Managerの分類を、画面が単一／一覧／選択かではなくeditable stateの所有単位へ訂正した。外部既存instanceまたはその場の新規instanceは単数Manager、配列と行選択dispatchは複数形Managerが所有し、相互に内包しない。複数形ManagerはAirArrayManagerの`beforeEdit`により内部editorまたは詳細navigation＋dialog抑止を選べる。Autocomplete内Createは単数ManagerのCREATEを使う。commit `9bd4d43c23bf113790add10691dc91b7445a99d4`のCustomer実装はAutocompleteを複数形Managerへ接続し、一覧もpageが選択から直接詳細routeへ迂回するため訂正対象である。旧Local記録は当時の履歴として保持し、480px、archive例外、listener・cache、Rules・data契約は変更しない。[現行判断](docs/decisions/0069-domain-manager-editable-state-ownership.md)、[旧判断](docs/decisions/0068-domain-manager-wrapper-and-editor-dialog-convention.md)、[ロードマップ](docs/roadmaps/foundational-governance-alignment.md)を参照。
