...
- FGA-06-SCHEDULE-MANAGER-RESTORE-09として、現場稼働予定の単数・複数Manager、配置作業員、複製、配置画面の保存を`AirItemManager`／`AirArrayManager`と`SiteOperationSchedule` modelへ戻した。予定と配置通知のRulesは同じ会社の有効な本登録Userによる通常read/writeへ簡素化し、Site revision、maintenance、live Site、field検査の重複を外した。予定から実績への確定は既存server処理を維持する。Dev受入れで上下番確定の左右画面が表示されない不具合を確認し、Managerの汎用表示転送から画面本体用の`table`を除外して、左の予定一覧と右の詳細・日報写真が上書きされないよう補正した。commit `12f05e5a`をGitHub ActionsでHostingへDev再反映し、画面再受入れを待つ。data shape、migration、schema package、Rules、Functions、Prodはこの補正で変更していない。[ADR 0073](docs/decisions/0073-schedule-manager-and-rules-restoration.md)、[Local検証記録](docs/verification/fga-06-schedule-manager-restore-local.md)、[Dev反映・受入れ記録](docs/verification/fga-06-schedule-manager-restore-dev.md)を参照。

- FGA-06-RESULT-CREATE-CLIENT-08として、一覧画面に残っていた`OperationResultsManager`の`OperationArrayManager`接続を過去実装に沿う`AirArrayManager`へ戻し、通常作成を`OperationResult.create()`へ接続した。現行の760px dialog、入力、ボタン文言、作成後の詳細遷移は維持する。Rulesは同一tenantの有効な本登録User、actor UID、document ID、live Site／Customer、空の作業員・稼働外売上・調整値、非lock、非予定紐付けを作成境界とし、旧Callableの実績create入力を拒否した。固定release `65a10bde`でFirestore Rules・Functions・HostingをDev反映し、会社管理者による一覧作成・詳細遷移・物理削除、一覧0件、Trigger errorなし、既存UIを受け入れた。複製、稼働外売上、請求、lock、予定、通知、実績化、schema、migration、Prodは変更していない。[Local検証記録](docs/verification/fga-06-result-create-client-local.md)と[Dev受入れ記録](docs/verification/fga-06-result-create-client-dev.md)を参照。

- FGA-06-RESULT-CALLABLE-RESTORE-07として、過去実装を基準に稼働実績の基本情報と作業員配列を`OperationResultManager`／複数形`WorkersManager`、Air Manager、`OperationResult.update()`へ戻した。後から追加された専用writer、Employee存在確認transaction、再読込handler、実績`overview`／`workers`／`delete` Callable入力とRulesのSite／Customer存在確認を撤去した。初回Dev受入れで確認した作業員の勤務初期値継承不足は、過去repositoryと現行component契約に合わせてcommit `8d90d5d1`で補正した。補正後に会社管理者sessionで作業員追加・更新・削除・再読込、実績物理削除、一覧0件、Trigger errorなし、既存UIを確認し、FGA-06-RESULT-DELETE-CLIENT-06とともに完了した。Prod、migration、既存data一括変更は行っていない。[Local検証記録](docs/verification/fga-06-result-callable-restore-local.md)と[Dev受入れ記録](docs/verification/fga-06-result-callable-restore-dev.md)を参照。
...
- Manager構成・customInput・編集dialog幅・document単位の同時更新の詳細を現行仕様へ集約し、development/data規則と開発workflowの参照方向を整理した。local UIは準備から終了までの順序へ整理し、coordinationと環境・承認規則の重複を参照へ統合した。さらにtenant・document分割・表示data・UI責務を仕様へ集約し、workflowを適用手順へ整理した。CONF/FUTの状態別参照を設け、古い集計と引継ぎを履歴として明示した。製品再開案内を残件・承認・正本への案内へ絞り、UWB・EmployeeとFGA内部表の古い適用状態を既存の完了記録へ整合した。Employee保険は世代値の保持・client計算時の増加と、server最新値との競合拒否撤去を分けて仕様・実装調査・ADRへ反映した。複数端末の順序・二重適用保護を今回検証済みとは扱わない。Employeeロードマップ、archive実装差、初回Dev反映・受入れ、Company設定、固定inventory精査計画の現行参照と履歴も分離した。Site・Outsourcerの旧工程と後続適用状態、確認事項依存表とcoverage監査・棚卸しの過去集計も整理した。Functions・Callableの旧調査と後続境界、Customerの閉鎖済み試験計画を整理し、全体ロードマップの逐次履歴を節目と詳細参照へ集約した。既存の回答・課題状態・例外・段階移行・承認・停止条件を維持する。[文書整理の検証記録](docs/verification/document-spec-authority.md)を参照。
...
- Customerの単数・複数形Managerをbase Air Managerの既定editor、form validation、mode管理、error eventへ戻し、独自`operation`／`open`／`editor`／固定snapshotを撤去した。単数activatorは`toCreate`／`toUpdate`をoperation別に公開し、listener由来instanceを直接渡して編集中draftも最新document全体へ置き換える。Customerは暫定的に`includedKeys`を維持し、通常dialog 480px、generic delete拒否、archive専用境界を維持する。`useBaseManager`は現行event契約へ適合するため標準logger・error message・loading接続に利用するが、全domain Managerへの使用を強制しない。sourceと自動testはlocalで確認し、利用者Local、Dev、Prod、remote data、Rules・Functions・schema・packageは未変更である。[現行判断](docs/decisions/0069-domain-manager-editable-state-ownership.md)と[ロードマップ](docs/roadmaps/foundational-governance-alignment.md)を参照。

- Domain Managerの分類をeditable stateの所有単位へ訂正し、Customer実装も追随した。外部既存instanceまたはその場の新規instanceは単数Manager、配列と行選択dispatchは複数形Managerが所有して相互に内包しない。この分類をFirestore上の通常のmaster data全般へ適用し、Customer・Site・Employee・Outsourcerを最初の適用例とした。Autocomplete内Createは単数`CustomerManager`、一覧の行選択は`CustomersManager`／AirArrayManagerの`beforeEdit`を経由し、詳細navigation時はfalseで一覧dialogを抑止する。独立review、対象自動検証、全domain検証、利用者Local確認を完了し、固定merge commitをHostingへDev反映した。会社管理者の外部Chromeで一覧CREATE、listener反映、詳細navigation、単数ManagerのUPDATE、両dialogの480pxと再読込後の値を確認した。Autocomplete Create runtimeは現行callerがなく未確認であり、Company・User等の例外、archive例外、listener・cache、Rules・data契約は変更していない。[現行判断](docs/decisions/0069-domain-manager-editable-state-ownership.md)、[Dev受入れ記録](docs/verification/fga-02-customer-manager-dev.md)、[ロードマップ](docs/roadmaps/foundational-governance-alignment.md)を参照。

- Customer詳細の基本情報・支払条件を共通`CustomerManager`／`AirItemManager`へ移し、Prod前のdocument単位last-write-winsに合わせて26field全体をmergeなしで置換する通常更新へ変更した。編集中の外部更新で保存を拒否・再読込要求せず、更新後はlistener受信値だけを表示正本とする。archive・作成・物理deleteは単数Managerから分離した。実Customer converterと26-field契約の一致、対象31件、domain 1,570件、Local Emulator 182件、独立reviewをlocalで確認した。Dev、Prod、remote data、Rules・Functions・schema・packageは未変更である。[実装調査](docs/implementation/customer-master.md)と[ロードマップ](docs/roadmaps/foundational-governance-alignment.md)を参照。
