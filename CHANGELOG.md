...
- FGA-06-SCHEDULE-MANAGER-RESTORE-09として、現場稼働予定の単数・複数Manager、配置作業員、複製、配置画面の保存を`AirItemManager`／`AirArrayManager`と`SiteOperationSchedule` modelへ戻した。予定と配置通知のRulesは同じ会社の有効な本登録Userによる通常read/writeへ簡素化し、Site revision、maintenance、live Site、field検査の重複を外した。予定から実績への確定は既存server処理を維持する。Dev受入れで上下番確定の左右画面が表示されない不具合を確認し、Managerの汎用表示転送から画面本体用の`table`を除外して、左の予定一覧と右の詳細・日報写真が上書きされないようLocal補正した。補正版のDev再反映と、配置管理・上下番確定処理そのものの再受入れは未実施。data shape、migration、schema package、Rules、Functions、Prodはこの補正で変更していない。[ADR 0073](docs/decisions/0073-schedule-manager-and-rules-restoration.md)、[Local検証記録](docs/verification/fga-06-schedule-manager-restore-local.md)、[Dev反映・受入れ記録](docs/verification/fga-06-schedule-manager-restore-dev.md)を参照。
