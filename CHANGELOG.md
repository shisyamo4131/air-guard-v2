...
- Domain Managerの分類をeditable stateの所有単位へ訂正し、Customer実装も追随した。外部既存instanceまたはその場の新規instanceは単数Manager、配列と行選択dispatchは複数形Managerが所有して相互に内包しない。Autocomplete内Createは単数`CustomerManager`、一覧の行選択は`CustomersManager`／AirArrayManagerの`beforeEdit`を経由し、詳細navigation時はfalseで一覧dialogを抑止する。独立review、対象自動検証、全domain検証と利用者Localの非書込み確認を完了した。Autocomplete Create runtimeとDev受入れは未完了であり、480px、archive例外、listener・cache、Rules・data契約は変更していない。[現行判断](docs/decisions/0069-domain-manager-editable-state-ownership.md)、[訂正後Local記録](docs/verification/fga-02-customer-manager-correction-user-local.md)、[ロードマップ](docs/roadmaps/foundational-governance-alignment.md)を参照。
