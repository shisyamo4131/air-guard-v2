...
- CCB-02のlocal-only pure migration planner、Firestore REST Valueのtype-tagged canonical digest、合成fixture、17件の回帰testを追加した。Schemas exact `2.4.2-dev.167`のlegacy mappingを使用し、8 targetのcreate-only、complete exactの再実行、partial・不一致・unknown・invalid・ambiguous・orphan・edition未確認での全体write 0とmanifest digest bindingを検証する。plannerはFirestore reader/writerを持たず、直接実行はusage errorで停止するため、Emulator、Dev/Prod、実data、networkへの作用はない。
