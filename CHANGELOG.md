...
- FGA-06-SCHEDULE-MANAGER-RESTORE-09として、現場稼働予定の単数・複数Manager、配置作業員、複製、配置画面の保存を`AirItemManager`／`AirArrayManager`と`SiteOperationSchedule` modelへ戻した。予定と配置通知のRulesは同じ会社の有効な本登録Userによる通常read/writeへ簡素化し、Site revision、maintenance、live Site、field検査の重複を外した。予定から実績への確定は既存server処理を維持する。data shape、migration、schema package、Dev・Prodは変更していない。配置管理と上下番確定で発生中のエラーは復元後の別checkpointで調査する。[ADR 0073](docs/decisions/0073-schedule-manager-and-rules-restoration.md)と[Local検証記録](docs/verification/fga-06-schedule-manager-restore-local.md)を参照。
