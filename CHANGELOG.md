...
- Customer、Site、Employee、Outsourcerの通常C/U/Dを利用文脈別のdomain Manager経由とし、単一documentは`AirItemManager`、collection・一覧・選択は`AirArrayManager`のwrapperへ統一するガバナンスを採用した。Autocomplete内Createはcommit成功と割当済みdocument IDを確認して即時選択でき、主対象document・listはlistener、従属先の名称・詳細だけはtenant scopeを分離した`useFetch`共有cacheを表示正本とする。通常data編集dialogは最大幅480pxを既定とし、例外operationは安全性・可読性・responsive・accessibilityに応じて個別に定める。Customerの追加wrapper、Create接続、archive入口整理、480px化とLocal・Dev受入れは未完了である。[判断](docs/decisions/0068-domain-manager-wrapper-and-editor-dialog-convention.md)と[ロードマップ](docs/roadmaps/foundational-governance-alignment.md)を参照。

- Customer詳細の基本情報・支払条件を共通`CustomerManager`／`AirItemManager`へ移し、Prod前のdocument単位last-write-winsに合わせて26field全体をmergeなしで置換する通常更新へ変更した。編集中の外部更新で保存を拒否・再読込要求せず、更新後はlistener受信値だけを表示正本とする。従来のdialog外観とrole別UXを維持し、archive・作成・物理deleteはManagerから分離した。実Customer converterと26-field契約の一致、対象31件、domain 1,570件、Local Emulator 182件、独立reviewをlocalで確認した。runtime UI、Dev、Prod、remote data、Rules・Functions・schema・packageは未変更である。[実装調査](docs/implementation/customer-master.md)と[ロードマップ](docs/roadmaps/foundational-governance-alignment.md)を参照。
