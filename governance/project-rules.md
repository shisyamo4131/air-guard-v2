...
- real-time listenerが更新するlive modelを入力中のdraftとして直接変更しない。editor開始時に独立draftを作り、購読値が編集中に変わっても入力を黙って上書きしない。同じoperation所有fieldの変更を検出した場合は利用者へ通知し、最新値の再読込またはlast-write-winsでの再確認を選べるようにする。
