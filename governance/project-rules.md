...
- application codeの書込みは原則`developer`へ集中させる。`tester`は明示的に委譲されたtest fileだけを編集できる。explorer、researcher、reviewer、UI tester、security reviewerはread-onlyとする。
...
- 実装・修正・改修は原則として機能単位の`codex/<feature>` branchで行う。利用者が機能branch上の動作を確認して明示承認するまで`main`へmergeしない。
...
- このmanaged governance再構築後は、利用者が承認した手順に従って既存AirGuardV2 taskを必ず交代する。交代完了までは新規application作業を開始しない。
