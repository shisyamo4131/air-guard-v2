...
import { reactivateSite as reactivateSiteUseCase } from "../modules/sites/lifecycle.js";
import { mapSiteLifecycleError } from "../modules/sites/mappers.js";

export const reactivateSite = onCall(async (request) => {
  ...
  try {
    ...
    return await reactivateSiteUseCase({ firestore: getFirestore(), identity, input: request.data });
  } catch (error) {
    logger.error("Site reactivation failed", { errorName: error?.name, errorCode: error?.code });
