...
export function mapSiteAgreementUpdateError(error) {
  const identity = mapCallableAuthIdentityError(error);
  if (identity) return identity;
  if (!(error instanceof SiteAgreementUpdateError)) {
    return { code: "internal", message: "取極めを保存できませんでした。" };
  }
  switch (error.code) {
    case SITE_AGREEMENT_ERROR_CODES.INVALID_INPUT:
      return { code: "invalid-argument", message: "取極めの入力内容を確認してください。" };
    case SITE_AGREEMENT_ERROR_CODES.ACTOR_NOT_ALLOWED:
      return { code: "permission-denied", message: "取極めを変更する権限がありません。" };
    case SITE_AGREEMENT_ERROR_CODES.SITE_NOT_FOUND:
      return { code: "not-found", message: "現場が見つかりません。" };
    case SITE_AGREEMENT_ERROR_CODES.CONFLICT:
      return { code: "aborted", message: "取極めが別の画面で更新されました。" };
    case SITE_AGREEMENT_ERROR_CODES.INVALID_STATE:
    case SITE_AGREEMENT_ERROR_CODES.MAINTENANCE:
