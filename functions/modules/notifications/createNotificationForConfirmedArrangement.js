...
/*****************************************************************************
 * 配置通知が "確認済み" に更新されたことを通知するための Notification ドキュメントを
 * 作成します。
 *****************************************************************************/
export async function createNotificationForConfirmedArrangement(
  arrangementData,
  companyId,
) {
  ...
  try {
    ...
    // → User ドキュメントのうち、`receiveConfirmedArrangementNotification` が true のものを対象とする
    const usersSnapshot = await db
      .collection(`Companies/${companyId}/Users`)
      .where("receiveConfirmedArrangementNotification", "==", true)
      .get();
    if (usersSnapshot.empty) {
      const message = `No users found who receive confirmed arrangement notifications.`;
      logger.log(message);
      return;
    }
    ...
    await db.collection(`Companies/${companyId}/Notifications`).add({
      title: siteName,
      body: `${employeeName}さんが配置を確認しました。`,
      data: {
        type: "arrangement",
    ...
    logger.log(
      `Notification document created for confirmed arrangement: ${arrangementData.docId}`,
    );
  } catch (error) {
    const message = `[createNotificationForConfirmedArrangement] Error creating notification for arrangementNotification: ${arrangementData.docId}`;
