...
/*****************************************************************************
 * 配置通知が "下番" に更新されたことを通知するための Notification ドキュメントを
 * 作成します。
 *****************************************************************************/
export async function createNotificationForLeavedArrangement(
  arrangementData,
  companyId,
) {
  ...
  try {
    ...
    // → User ドキュメントのうち、`receiveLeavedArrangementNotification` が true のものを対象とする
    const usersSnapshot = await db
      .collection(`Companies/${companyId}/Users`)
      .where("receiveLeavedArrangementNotification", "==", true)
      .get();
    if (usersSnapshot.empty) {
      const message = `No users found who receive leaved arrangement notifications.`;
      logger.log(message);
      return;
    }
    ...
    logger.log(
      `Notification document created for leaved arrangement: ${arrangementData.docId}`,
    );
  } catch (error) {
    const message = `[createNotificationForLeavedArrangement] Error creating notification for arrangementNotification: ${arrangementData.docId}`;
